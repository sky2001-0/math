...
\lem{実数の準備：前順序の全域性}{
  ...
  $\qty(x_n - y_n)_{n \in \N}$は、\lemref{可換位相群の一様的な特徴}よりCauchy列であるので、
  \eq*{
...
\lem{実数の準備：前順序の補足}{
  ...
  \lemref{可換位相群の一様的な特徴}より$\qty(x_l - y_l)_{l \in \N}$はCauchyである。
...
\lem{実数の準備：加法可換群}{
  ...
  \lemref{可換位相群の一様的な特徴}よりマグマである。

  結合法則は、有理数の可換体としての性質より明らか。

  $\qty(0)_{n \in \N}$は単位元である。

  $\qty(x_n)_{n \in \N}$に対して、\lemref{可換位相群の一様的な特徴}より$\qty(-x_n)_{n \in \N}$は逆元である。
...
\lem{実数の準備：両立}{
  ...
  $\qty(x_n + y_n) - \qty(x'_n + y'_n) = \qty(x_n - x'_n) + \qty(y_n - y'_n)$であるので、$0$に収束する。\\*
...
\lem*{
  ...
  以下の写像は、全単射$f \in \R^{\outsqbr{0, 1}}$である。
  \eq*{
    f(x) \coloneqq \frac{1 / 2 - x}{x \qty(x - 1)}
  }

  以下の写像は、全単射$g \in \outsqbr{0, 1}^{\qty[0, 1]}$である。
  \eq*{
    g(x) \coloneqq
    \begin{cases}
...
\thm{連続体濃度}{
  ...
  $x_n \coloneqq \sum_{m \in n} \qty(y_m / 2^m)$とすると、$\qty(x_n)_{n \in \N}$は$\Q$上のCauchy列である。
...
\thm{有界と全有界}{
  ...
  \thmref{順序群の一様構造}より$A \subseteq \qty{B(\epsilon)[d + m \times \epsilon] \mid m \in s(n)}$である。
}
...
\lem*{
  ...
  \thmref{閉包}より$\exists \epsilon \in \R^+ \qty(A \cap \outsqbr{\sup A - \epsilon, \sup A + \epsilon} = \varnothing)$である。
...
\thmf{$\R$は連結}{Rは連結}{
  ...
  $b \in V$とすると、$V$は開集合であるので$\exists \epsilon \in \R^+ \qty(\outsqbr{b - \epsilon, b + \epsilon} \subseteq V)$

  このとき$b - \epsilon$も$B_1$の上界となり矛盾。
  ゆえに$b \notin V$
  よって$b < v$である。

  $b \in U$とすると、$U$は開集合であるので$\exists \epsilon' \in \R^+ \qty(\outsqbr{b - \epsilon', b + \epsilon'} \subseteq U)$
