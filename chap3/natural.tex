...
\dfn{自然数}{
  自然数論では、\num{3}つの公理（
    \axiref{零は後続ではない}、\axiref{後続は単射}、\axiref{数学的帰納法}
  ）が与えられる。
...
\thm{Peanoの公理}{
  \thmref{最小帰納的集合の一意性}より、帰納的集合$X$の取り方によらず$\omega(X)$が一意に定まる。
  これを$\N$で表す。
  ...
  \eqg*{
    ...
    \forall E \qty(E \subseteq \N \land \varnothing \in E \land \forall n \qty(n \in E \rightarrow n^+ \in E) \rightarrow E = \N)
  }
}{
  第一、第二の公理は、帰納的集合であることより明らか。
  ...
  第四の公理は、\lemref{後続は単射的}より示される。

  \vskip\baselineskip

  第五の公理を示す。

  前件より、$E \subseteq \N$である。

  $\varnothing \in E \land \forall n \qty(n \in E \rightarrow n^+ \in E)$より、$E$は帰納的集合である。

  最小帰納的集合であるので、$\N = \omega(E) \subseteq E$である。
}
...
\lem*{
  ...
  ある$n$が順序数であるとき、\corref{順序数の後続は順序数}より$s(n)$は順序数である。
...
\lem{自然数の順序の加法による保存}{
  ...
  ある自然数$l$で成り立つとき、\lemref{順序数の後続と比較}より、
  \eq*{
...
\lem{自然数の順序の加法による特徴づけ}{
  ...
  $n < m$のとき、\lemref{順序数の後続と比較}より$s(n) \leq m$である。
