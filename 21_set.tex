...
\dfn{要素}{
  集合$x, X$について、$x \in X$であるとき、$x$は$X$の要素または元と言う。
}
...
\dfn{包含}{
  集合$X, Y$が以下を満たす時、$X$は$Y$の部分集合であると言い、$X \subset Y$と表す。
  \eq*{
...
\dfn{単集合}{
  $x = y$を考えることで、集合$\qty{x, x}$が存在することが言える。\axiref{外延性の公理}から、$\qty{x}$と表記して良い。\\*
...
\rem{集合の内包的定義１}{
  \axiref{外延性の公理}より、\axiref{置換の公理図式}より主張される集合$A$は一意に定まる。\axiref{置換の公理図式}に基づく定義方法を、内包的定義と呼び以下のように表す。
...
\cor*{
  \eq*{
    \forall X \forall Y \qty(\P(X) \cap \P(Y) = \P \qty(X \cap Y))
  }
}
...
\lem*{
  \eq*{
    \forall X \forall Y \forall x \forall y \qty(x \in X \land y \in Y \rightarrow \qty(x, y) \in \P \qty(\P \qty(X \cup Y)))
  }
}{
  \eqg*{
    x \in X \land y \in Y \\*
    \qty{x} \in \P \qty(X \cup Y) \land \qty{x, y} \in \P \qty(X \cup Y) \\*
    \qty(x, y) \in \P \qty(\P \qty(X \cup Y))
  }
  よって従う。
}
...
\dfn{直積集合}{
  ...
  \eq*{
    X \times Y \coloneqq \qty{z \in \P \qty(\P \qty(X \cup Y)) \mid \exists x \exists y \qty(x \in X \land y \in Y \land z = \qty(x, y))}
  }
}
