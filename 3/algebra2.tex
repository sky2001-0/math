...
\thm{Sylowの存在定理}{
	...
	$\abs{G}$が$p^n$の倍数である任意の有限群$G$について、満たす部分群$H$が存在することを数学的帰納法により示す。

	$n = 0$のとき、$\qty{e_G}$は$G$の部分群である。\\*

  ある$n$について成り立つとする。$\abs{G}$が$p^{s(n)}$の倍数となるような群$G$を考える。

  $G$の$G$への共役作用$\varphi_C$と、$G$の中心$\Fix(G, G)$を考える。\\*
	
	$\abs{\Fix(G, G)}$が$p$の倍数でないとき、\thmref{類等式}より$\abs{G} = \abs{\Fix(G, G)} + \sum_{m \in n} \abs{\varphi(G, g_m)}$であるので、$\exists g \in G$であり$\abs{\varphi(G, g)}$は$p$の倍数ではない。

	\thmref{軌道・安定化部分群定理}、\lemref{Lagrangeの定理}より、$\abs{G} = \abs{G / \Stab(G, g)} \abs{\Stab(G, g)} = \abs{\varphi(G, g)} \abs{\Stab(G, g)}$である。

	ゆえに、$\abs{\Stab(G, g)}$は$p^{s(n)}$の倍数となる。\\*
  ...
  \lemref{Cauchyの定理}より、$\Fix(G, G)$は部分群$N$を持って$\abs{N} = p$であり、$\Fix(G, G)$が$G$の正規部分群であることから、$N$は$G$の正規部分群である。
	...
	剰余群$G / N$について、\lemref{Lagrangeの定理}より$\abs{G} = \abs{G / N} \abs{N}$であり、$\abs{G / N}$は$p^n$の倍数であるので、仮定より部分群$K$が存在して、$\abs{K} = p^n$である。

	\lemref{剰余群の部分群は部分群の剰余群}より、$G$の部分群$L$が存在して$L / N = K$であり、\lemref{Lagrangeの定理}より$\abs{L} = \abs{L / N} \abs{N} = \abs{K} \abs{N} = p^{s(n)}$
}
