...
\lem{自然数は最小の無限順序数}{
  有限でない順序数$\alpha$について、$\N \subset \alpha$である。
}{
  自然数は順序数であるので、任意の$n \in \N$について、\lemref{順序数の比較可能性}より$n \in \alpha \lor \alpha \subset n$である。

  $\alpha \subset n$であるとき、\lemref{単射と有限}より$\alpha$は有限となり、反する。

  したがって、$n \in \alpha$である。

  ゆえに、$\N \subset \alpha$である。
}
...
\lem{無限順序数の後続の濃度}{
  ...
  \lemref{自然数は最小の無限順序数}より、$\N \subset \alpha$である。
...
\lem{無限順序数の直積}{
  ...
  \eq*{
    \qty(\min A \times \min A)_{< \qty(\xi_0, \xi_1)} \subset \xi_2 \times \xi_2
  }

  \lemref{順序数の後者と比較}より、$\xi_2 \in \min A \lor \xi_2 = \min A$であり、$\xi_2 = \min A$のとき\lemref{無限順序数の後続の濃度}より単射$\min A \to \max{\xi_0, \xi_1}$が存在しないことに反する。
