...
\lem{全射と要素数}{
  ...
  $\Hom[\mathbf{Set}](s(n), 0) = \varnothing$より、$m \neq 0$である。
...
\dfn{要素数}{
  有限集合$X$について、\lemref{要素数の一意性}より主張される自然数を要素数$\abs{X}$と呼ぶ。

  また、集合$X$が有限集合であることを主張するときは、アリティ1の述語$\abs{X} < \infty$を用いる。
}
...
\lem{単射と有限}{
  ...
  全単射$f \colon A \to \abs{A}$が存在して、単射$g \colon \abs{A} \to n$が存在する。

  ゆえに、単射$g \circ f \colon A \to n$が存在する。

  \vskip\baselineskip

  $2. \rightarrow 1.$を示す。

  $n = 0$のとき、$\Hom[\mathbf{Set}](A, n) \neq \varnothing$より、$A = \varnothing$である。
...
\dfn{可算}{
  ...
  また、集合$X$が可算集合であることを主張するときは、アリティ1の述語$\abs{X} \leq \aleph_0$を用いる。
}
...
\lem{自然数は最小の無限順序数}{
  有限でない順序数$\alpha$について、$\N \subseteq \alpha$である。
}{
  自然数は順序数であるので、任意の$n \in \N$について、\lemref{順序数の比較可能性}より$n \in \alpha \lor \alpha \subseteq n$である。

  $\alpha \subseteq n$であるとき、\lemref{単射と有限}より$\alpha$は有限となり、反する。

  したがって、$n \in \alpha$である。

  ゆえに、$\N \subseteq \alpha$である。
}

\thm{無限集合は可算部分を含む}{
  有限でない集合$X$について、単射$\N \to X$が存在する。
}{
  \thmref{整列可能定理}と\lemref{整列集合は順序型を持つ}より、順序数$\alpha$が存在して全単射$\varphi \colon \alpha \to X$を持つ。

  \lemref{自然数は最小の無限順序数}より、$\varphi \circ \iota_{\N, \alpha}$は求める単射である。
}

\lem{無限順序数の後続の濃度}{
  有限でない順序数$\alpha$について、単射$\alpha^+ \to \alpha$が存在する。
}{
  \lemref{自然数は最小の無限順序数}より、$\N \subseteq \alpha$である。

  以下で定める$\varphi \colon \alpha^+ \to \alpha$は単射である。
  \eq*{
    \varphi(x) \coloneqq
    \begin{cases}
      0 & \qty(x = \alpha) \\*
...
\lem{無限順序数の直積}{
  有限でない順序数$\alpha$について、全単射$\alpha \times \alpha \to \alpha$が存在する。
}{
  ある有限でない順序数$\alpha$について、全単射$\alpha \times \alpha \to \alpha$が存在しないとする。

  $A \coloneqq \qty{\beta \in \alpha^+ \mid \lnot \qty(\abs{\beta} < \infty) \land \text{単射$\beta \times \beta \to \beta$が存在しない。}}$を考える。

  $\alpha \in A$より$A \neq \varnothing$であるので、$\min A$が存在する。

  \vskip\baselineskip

  任意の$\beta \in \min A$について、単射$\min A \to \beta$が存在しないことを示す。

  $\beta$が有限のとき、単射$\min A \to \beta$が存在するならば\lemref{単射と有限}より$\min A$は有限集合となり、$\min A \in A$に反する。

  $\beta$が有限でないときを考える。

  存在するとすると、$\iota_{\beta, \min A}$は単射より、\thmref{Cantor-Schroder-Bernsteinの定理}より全単射$\min A \to \beta$が存在する。

  単射$\beta \times \beta \to \beta$が存在するとすると、単射$\min A \times \min A \to \min A$を構成できるので、単射$\beta \times \beta \to \beta$は存在しない。

  ゆえに、$\beta \in A$の最小性に反する。

  よって任意の$\beta \in \min A$について、単射$\min A \to \beta$が存在しない。

  \vskip\baselineskip

  以下で定める$\min A \times \min A$上の関係$\leq$について、$\qty(\min A \times \min A, \leq)$は整列集合である。
  \eqa*{
    &\qty(\xi_0, \xi_1) \leq \qty(\eta_0, \eta_1) \\*
    &\defiff \max{\xi_0, \xi_1} < \max{\eta_0, \eta_1} \lor \qty\Big(\max{\xi_0, \xi_1} = \max{\eta_0, \eta_1} \land \qty\big(\xi_0 < \eta_0 \lor \qty(\xi_0 = \eta_0 \land \xi_1 \leq \eta_1)))
  }

  \vskip\baselineskip

  任意の$\qty(\xi_0, \xi_1) \in \min A \times \min A$について、$\xi_2 \coloneqq \max{\xi_0, \xi_1}^+$として、以下が成り立つ。
  \eq*{
    \qty(\min A \times \min A)_{< \qty(\xi_0, \xi_1)} \subseteq \xi_2 \times \xi_2
  }

  \lemref{順序数の後続と比較}より、$\xi_2 \in \min A \lor \xi_2 = \min A$であり、$\xi_2 = \min A$のとき\lemref{無限順序数の後続の濃度}より単射$\min A \to \max{\xi_0, \xi_1}$が存在しないことに反する。

  ゆえに、$\xi_2 \in \min A$である。

  $\xi_2 \notin A$より、$\xi_2$は有限である、または、単射$\xi_2 \times \xi_2 \to \xi_2$が存在する。

  $\xi_2$が有限であるとき、\lemref{有限集合の直積}より$\xi_2 \times \xi_2$も有限であるので、\lemref{単射と有限}より単射$\min A \to \xi_2 \times \xi_2$は存在しない。

  単射$\xi_2 \times \xi_2 \to \xi_2$が存在するとき、$\xi_2 \in \min A$より単射$\min A \to \xi_2$は存在しないので、単射$\min A \to \xi_2 \times \xi_2$は存在しない。

  したがって、全射$\qty(\min A \times \min A)_{< \qty(\xi_0, \xi_1)} \to \min A$は存在しない。

  \vskip\baselineskip

  \lemref{整列集合は順序型を持つ}より、順序数$\alpha_2$が一意に存在して、順序同型$\varphi \colon \qty(\min A \times \min A, <) \to \qty(\alpha_2, \in)$が存在する。

  上の議論より、$\forall \xi \in \alpha_2 \qty(\text{全射$\xi \to \min A$が存在しない。})$であるので、\lemref{順序数の濃度の上限}から単射$\alpha_2 \to \min A$が存在する。

  ゆえに、単射$\min A \times \min A \to \min A$が存在して、反する。
}

\thm{無限濃度の直積}{
  有限でない集合$X$について、全単射$X \times X \to X$が存在する。
}{
  \thmref{整列可能定理}、\lemref{整列集合は順序型を持つ}、\lemref{無限順序数の直積}より明らかである。
}
