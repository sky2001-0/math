...
\thm{位相群の左一様構造}{
  ...
  \dfnref{基本近傍系}第三式より$\exists B \in \mathcal{B}(e) \qty(B \subseteq B_1 \cap B_2)$であり、定義より$\tilde{B} \subseteq V_1 \cap V_2 \land \tilde{B} \in \mathcal{V}$\\*

  $\forall V \in \mathcal{V} \exists B \in \mathcal{B}(e) \qty(V = \tilde{B})$である。

  \thmref{位相群の自明な連続写像}第二と\thmref{連続と点連続}より$\exists B_1, B_2 \in \mathcal{B}(e) \qty(\cdot(B_1 \times B_2) \subseteq B)$

  \dfnref{基本近傍系}第三式より$\exists B' \in \mathcal{B}(e) \qty(\cdot(B' \times B') \subseteq \cdot(B_1 \times B_2) \subseteq B)$

  定義より$\tilde{B'} \circ \tilde{B'} \subseteq \tilde{\cdot \qty(B' \times B')} \subseteq \tilde{B} = V$\\*

  $\forall V \in \mathcal{V} \exists B \in \mathcal{B}(e) \qty(V = \tilde{B})$である。

  \thmref{位相群の自明な連続写像}第一と\thmref{連続と点連続}より$\exists B' \in \mathcal{B}(e) \qty(-1(B') \subseteq B)$となるので、$\tilde{B'} \subseteq V^{-1}$\\*

  $\mathcal{V}$の与える基本近傍系は$\mathcal{B'}(x) = \qty{\tilde{B}[x] \mid B \in \mathcal{B}(e)}$である。\\*

  \thmref{位相群の自明な連続写像}第二と\lemref{直積と連続}より$\forall D \in \mathcal{B}(x) \exists B \in \mathcal{B}(e) \qty(\qty{x y \mid y \in B} \subseteq D)$

  $\tilde{B}[x] = \qty{y \in G \mid x^{-1} y \in B} = \qty{x z \mid z \in B} \subseteq D$\\*

  \thmref{位相群の自明な連続写像}第二と\lemref{直積と連続}より$\forall B \in \mathcal{B}(e) \exists D \in \mathcal{B}(x) \qty(\qty{x^{-1} y \mid y \in D} \subseteq B)$

  $\tilde{B}[x] = \qty{y \in G \mid x^{-1} y \in B} \supseteq \qty{z \in G \mid x^{-1} z \in \qty{x^{-1} y \mid y \in D}} = D$
...
\lem{可換位相群の一様的な特徴}{
  ...
  $\forall V \in \mathcal{V}$について、\lemref{基本近縁系の三角性}より$\exists W \in \mathcal{V} \qty(W \cup W^{-1} \subseteq V)$である。
  ...
  $\forall V \in \mathcal{V}$について、$\exists W \in \mathcal{V} \qty(W \circ W \subseteq V)$
...
\lem{位相体の収束ネットの逆}{
  ...
  Hausdorffと収束の定義より$\exists B_0 \in \mathcal{B}(x) \exists \lambda_0 \in \Lambda \qty(0_F \notin B_0 \land \qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_0}} \subseteq B_0)$
...
\thm{順序群は位相群}{
  ...
  $\forall a, b \in G \qty(\varphi(x, y) \in \outsqbr{a, b})$を考える。\\*

  $G^+$が最小元$m$を持つとき、$\qty{x} = \outsqbr{x - m, x + m} \subseteq \outsqbr{a, b}$である。
  同様に$\qty{y} \subseteq \outsqbr{a, b}$でもある。

  $\forall B \in \mathcal{B}(\varphi(x, y)) \qty(\varphi(\qty{x} \times \qty{y}) = \qty{\varphi(x, y)} \subseteq B)$である。\\*

  $G^+$が最小元を持たないとき、\lemref{順序群の三角性}より$\exists g \in G^+ \qty(g + g \leq \min \qty{\varphi(x, y) - a, b - \varphi(x, y)})$である。

  ゆえに、$\varphi \qty(\outsqbr{x - g, x + g} \times \outsqbr{y - g, y + g}) \subseteq \outsqbr{a, b}$である。\\*

  \dfnref{順序空間}の定める開基$\mathcal{B}$について、\lemref{最大がない場合の順序の開基}、\lemref{非自明な順序群は最大を持たない}より、$\forall B \in \mathcal{B} \exists a, b \in G \qty(\outsqbr{a, b} = B)$である。
...
\thm{順序群の一様構造}{
  ...
  さらに、基本近縁系$\mathcal{V}$の定める近縁系$\mathcal{U'}$は、\thmref{位相群の左一様構造}の定める近縁系$\mathcal{U}$に一致する。
	...
	$\forall g \in G^+ \exists B \in \mathcal{B}(0_G) \qty(B = \outsqbr{-g, g} \land \tilde{B} = V_g)$である。

	$\forall B \in \mathcal{B}(0_G)$について、$B = \outsqbr{a, b} \land a < 0_G < b$である。$g = \min \qty{-a, b} \in G^+$について、$V_g \subseteq \tilde{B}$である。
...
\thm{順序群では全有界ならば有界}{
  ...
  全有界より$\exists Y \in \P(A) \qty(\abs{Y} < \infty \land A \subseteq \qty{B(g)[y] \mid y \in Y})$である。
...
\thm{順序環は位相環}{
  ...
  $\forall B \in \mathcal{B}(0_R) \exists a, b \in R \qty(0_R \in \outsqbr{a, b} = B)$である。

  \lemref{順序環の三角性}より、$\exists r \in R^+ \qty(r \times r \leq \min \qty{-a, b})$である。

  $B' \coloneqq \outsqbr{-r, r} \times \outsqbr{-r, r} \mathcal{B} \times \mathcal{B}$であり、$\times(B') \subseteq B$\\*

  $\forall x_0 \in R$について、$f \in R^R, f(x) = x \times x_0$を考える。

  $x_0 = 0_R$のとき、定値写像より連続。

  $x_0 \neq 0_R$のとき、点$0_R$について考える。$\forall B \in \mathcal{B}(0_R) \exists a, b \in R \qty(0_R \in \outsqbr{a, b} = B)$である。

  $r \coloneqq \min \qty{-a, b} / \abs{x_0}$について、$B' \coloneqq \outsqbr{-r, r}$であり、$f(B') \subseteq B$\\*
...
\thm{順序体は位相体}{
  ...
  開基$B = \outsqbr{a, b} \cap F^\times$を考える。

  $B = \varnothing$のとき、${-1}^{-1}(B) = \varnothing \in \mathcal{O}$である。

  $0 < a < b$のとき、${-1}^{-1}(B) = \outsqbr{1 / b, 1 / a} \in \mathcal{O}$である。
