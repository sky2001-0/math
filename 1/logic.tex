...
\dfn{矛盾}{
  あらゆる解釈において偽を与える命題を、矛盾と呼び、$\bot$で表す。
}
...
\lem{否定における命題の代入原理}{
  命題$\phi, \psi, \chi$について、以下が成り立つ。
  \eq*{
    \lnot \phi \land \qty(\phi \leftrightarrow \psi) \vdash \lnot \psi
  }
}{
  $\lnot \phi \defiff \phi \rightarrow \bot$であるので、
  \lemref{含意における命題の代入原理}より成り立つ。
}
...
\thm{選言の除去則}{
  ...
  $\phi \vdash \chi$と$\chi \vdash \lnot \lnot \chi$から$\phi \vdash \lnot \lnot \chi$である。

  ゆえに$\lnot \chi \land \phi \vdash \bot$であるので、$\lnot \chi \vdash \lnot \phi$である。
...
\axi{全称の導入則}{
  項$x$を出現させない命題$\phi$、項$x$を出現させないアリティ\num{1}の述語$\psi$について、以下を定める。
  \eq*{
...
\thm{項の入れ替え}{
  ...
  \eq*{
    \forall x \qty(\phi(x)) \vdash \forall y \qty(\phi(y))
  }
}{
  \axiref{全称の除去則}より、$\forall x \qty(\phi(x)) \vdash \phi(y)$である。

  $\forall x \qty(\phi(x))$は項$y$を出現させないので、\axiref{全称の導入則}より、$\forall y \qty(\phi(y))$を得る。
}
...
\thm{存在の除去則}{
  ...
  $\chi, \lnot \psi$は項$x$を出現させないので、$\forall x \qty(\lnot \phi(x))$である。

  $\exists y \qty(\phi(y))$を仮定すると、$\exists y \qty(\phi(y)) \defiff \lnot \forall x \qty(\lnot \phi(x))$であるため矛盾するので、\thmref{背理法}より$\psi$を得る。
}
...
\thm{酒場の法則}{
  アリティ\num{1}の述語$\phi$について、以下が成り立つ。
  \eq*{
    \vdash \exists x \qty(\phi(x) \rightarrow \forall y \qty(\phi(y)))
  }
}{
  略。
}
...
\axi{等号の代入原理}{
  アリティ\num{2}の述語$\phi$、項$x, y$について、以下を定める。
  \eq*{
    \phi(x, x) \land x = y \vdash \phi(x, y)
  }
}
...
\rem{存在の誤謬}{
  類$X$とアリティ\num{1}の述語$\phi$について、
  $\forall x \in X \qty(\phi(x))$から$\exists x \in X \qty(\phi(x))$を導くことはできない。
