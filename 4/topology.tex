...
\dfn{開基}{
  空でない集合$X$について、以下を満たす集合系$\mathcal{B} \subset \P(X)$を開基と呼ぶ。
  \eqg*{
...
\lem{開集合系の一意性}{
  集合$X$の開基$\mathcal{B}$と集合系$\mathcal{B'} \subset \P(X)$について、以下を満たすとき、$\mathcal{B'}$は開基である。
  \eqg*{
    \forall B' \in \mathcal{B'} \forall x \in B' \exists B \in \mathcal{B} \qty(x \in B \land B \subset B') \\*
    \forall B \in \mathcal{B} \forall x \in B \exists B' \in \mathcal{B'} \qty(x \in B' \land B' \subset B)
  }

  さらに$\mathcal{B}$の定める開集合系と、$\mathcal{B'}$の定める開集合系、この\num{2}つは一致する。
}{
...
\dfn{連続}{
  位相空間$\qty(X, \mathcal{O}), \qty(X', \mathcal{O'})$について、写像$f \colon X \to X'$が以下を満たすとき、$f$は連続であると呼ぶ。
  \eq*{
...
\lem{開基と連続}{
  位相空間$\qty(X, \mathcal{O}), \qty(X', \mathcal{O'})$と$X'$の開基$\mathcal{B'}$、および写像$f \colon X \to X'$を考える。
  このとき、以下の\num{2}つは同値である。
  \begin{enumerate}
    \item $f$は連続。
...
\dfn{同相}{
  位相空間$X, X'$と写像$f \colon X \to X'$について、$f$が全単射かつ$f$と$f^{-1}$がともに連続であるとき、$f$を同相写像と呼ぶ。

  また、同相写像$f \colon X \to X'$が存在するとき、$X$と$X'$は位相同型または同相であると呼ぶ。
}
...
\lem*{
  空でない集合$X$と、位相空間$\qty(X', \mathcal{O'})$、$\mathcal{O'}$を与える$X$の開基$\mathcal{B'}$、写像$f \colon X \to X'$を考える。
  ...
  第三式を考える。\thmref{原像の性質}と\thmref{開集合系の公理}第三式より$\forall A' \subset \mathcal{O'} \qty(\bigcup  \qty{f^{-1}(O') \mid O' \in A'} = f^{-1}(\bigcup A') \in \mathcal{O}_2)$
...
\dfn{誘導空間}{
  空でない集合$X$と、位相空間$\qty(X', \mathcal{O'})$、写像$f \colon X \to X'$について、\mlemref{0}より定まる開集合系$\mathcal{O}$が存在する。
...
\dfn{部分空間}{
  ...
  $f = \id_X \rvert_A$に誘導された位相空間$\qty(A, \mathcal{O}_A)$を部分空間と呼ぶ。
}
...
\lem*{
  ...
  さらに、$\mathcal{B}_1, \mathcal{B}_2$の与える開集合系と、$\mathcal{O}_1, \mathcal{O}_2$からこの補題により与える開基の定める開集合系、この\num{2}つは一致する。
}{
  第一式を考える。
...
\lem{直積と連続}{
  位相空間$\qty(X_1, \mathcal{O}_1), \qty(X_2, \mathcal{O}_2), \qty(Y, \mathcal{O})$について、連続写像$f \colon X_1 \times X_2 \to Y$を考える。

  このとき、$\forall w \in X_1$について写像$f_w \colon X_2 \to Y, f_w(x) = f(w, x)$は連続である。
}{
...
\dfn{基本近傍系}{
  空でない集合$X$とその任意の点$x$について、以下を満たす集合系$\mathcal{B}(x) \subset \P(X)$を基本近傍系と呼ぶ。
  \eqg*{
    \mathcal{B}(x) \neq \varnothing \\*
...
\lem{近傍系の一意性}{
  空でない集合$X$の基本近傍系$\mathcal{B}(x)$と集合系$\mathcal{B'}(x) \subset \P(X)$について、以下を満たすとき、$\mathcal{B'}(x)$は基本近傍系である。
  \eqg*{
    \forall B' \in \mathcal{B'}(x) \exists B \in \mathcal{B}(x) \qty(B \subset B') \\*
    \forall B \in \mathcal{B}(x) \exists B' \in \mathcal{B'}(x) \qty(B' \subset B)
  }

  さらに$\mathcal{B}(x)$の定める近傍系と、$\mathcal{B'}(x)$の定める近傍系、この\num{2}つは一致する。
}{
...
\thm{開基の定める基本近傍系}{
  ...
  さらに$\mathcal{B}(x)$の与える近傍系と、$\mathcal{O}$からこの定理の与える基本近傍系$\mathcal{B'}(x)$が与える近傍系、この\num{2}つは一致する。
}{
...
\thm{基本近傍系の定める位相}{
  ...
  さらに$\mathcal{O}$と、$\mathcal{N}(x)$からこの定理の与える開集合系$\mathcal{O'}$、この\num{2}つは一致する。
}{
  ...
  第三式を考える。$A \subset \mathcal{O}$を考える。
...
\lem*{
  ...
  $X$の開基$\mathcal{B}$の定める開集合系$\mathcal{O}$と、\thmref{開基の定める基本近傍系}と\thmref{基本近傍系の定める位相}により定まる開集合系$\mathcal{O'}$は、一致する。

  \vskip\baselineskip

  $X$の基本近傍系$\mathcal{B}(x)$について、\thmref{基本近傍系の定める位相}と\thmref{開基の定める基本近傍系}の定める基本近傍系$\mathcal{B'}(x)$を考える。

  $\mathcal{B}(x)$の定める近傍系と、$\mathcal{B'}(x)$の定める近傍系は、一致する。
}{
  以下より一致する。
  \eq*{
    \mathcal{O'} = \qty{O \subset X \mid \forall x \in O \exists B \in \mathcal{B}(x) \qty(B \subset O)} = \qty{O \subset X \mid \forall x \in O \exists B \in \mathcal{B} \qty(x \in B \land B \subset O)} = \mathcal{O}
  }
  ...
  定義より、以下である。
  \eq*{
    \mathcal{B'}(x) = \qty{B' \subset X \mid x \in B' \land \forall y \in B' \exists D \in \mathcal{B}(y) \qty(D \subset B')}
  }
...
\dfn{点連続}{
  位相空間$X, X'$と点$x \in X$、写像$f \colon X \to X'$について、以下を満たすとき、写像$f$は点$x$で連続であると言う。
  \eq*{
...
\thm{点連続と収束}{
  位相空間$X, X'$と点$x \in X$、写像$f \colon X \to X'$と、近傍系$\mathcal{N}(x)$を与える基本近傍系$\mathcal{B}(x)$について、以下の３つは同値である。
  \begin{enumerate}
    \item $f$が$x$で連続
...
\lem{コンパクトの言い換え}{
  ...
  \eq*{
    \forall A \subset \mathcal{F} \qty(A \neq \varnothing \land \bigcap A = \varnothing \rightarrow \exists A' \subset A \qty(A' \neq \varnothing \land \bigcap A' = \varnothing \land \abs{A'} < \infty))
  }
}{
...
\thm{コンパクト空間の連続像はコンパクト}{
  コンパクトな位相空間$\qty(X, \mathcal{O})$と、位相空間$\qty(X', \mathcal{O'})$について、連続写像$f \colon X \to X'$を考える。
  ...
  コンパクト性から、$\exists A' \subset C' \qty(\abs{A'} < \infty \land X = \bigcup \qty{f^{-1}(O') \mid O' \in A'})$
...
\thm{Hausdorff性}{
  ...
  \thmref{選択公理が与える写像}より、写像$g \colon \mathcal{N}(x) \times \mathcal{N}(y) \to X, g \qty(\qty(N_x, N_y)) \in N_x \cap N_y$が存在する。
...
\lem*{
  $T_2$空間$X$について、$X$のコンパクト部分$A$は閉集合である。
}{
  ???
}
...
\thm{連結空間の連続像は連結}{
  連結な位相空間$\qty(X, \mathcal{O})$と、位相空間$\qty(X', \mathcal{O'})$について、連続写像$f \colon X \to X'$を考える。
  このとき、像$f(X)$は連結である。
}{
  $f(X)$が連結でないと仮定する。
...
\thm{第一可算空間における点連続と収束}{
  第一可算な位相空間$X$、位相空間$X'$、点$x \in X$、写像$f \colon X \to X'$について、以下の\num{2}つは同値である。
  \begin{enumerate}
    \item $f$が$x$で連続
  ...
  \thmref{選択公理が与える写像}より、写像$g \colon \N \to X, g(n) \in B_n(x) \setminus f^{-1}(N')$が存在する。
...
\lem{最大がない場合の順序の開基}{
  ...
  このとき、\lemref{順序の開基}から得る開集合系と、$\mathcal{B'}$の定める開集合系、この\num{2}つは一致する。
}
