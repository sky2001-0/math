...
\thm{基底の存在}{
  ...
  $\mathcal{S} \coloneqq \qty{S \in \P(V) \mid \text{$S$は一次独立}}$について$\qty(\mathcal{S}, \subseteq)$は半順序集合である。
...
\lem{次元}{
  ...
  $P \coloneqq \qty{\varphi \in \bigcup \qty{B_2^D \mid D \in \P(B_1) \land B_1 \cap B_2 \subseteq D} \mid \varphi \rvert_{B_1 \cap B_2} = \id_{B_1 \cap B_2} \land C(\varphi) \coloneqq \dom(\varphi) \cup \qty(B_2 \setminus \Im(\varphi)) \text{は一次独立}}$を考える。

  ここで$p \in B_2^{B_1 \cap B_2}$について、$B_2$が基底であることから$p \in P$である。

  $P$上の半順序$\preccurlyeq$を以下のように定義する。
  \eq*{
    \varphi \preccurlyeq \psi \defiff \dom(\varphi) \subseteq \dom(\psi) \land \psi \rvert_{\dom(\varphi)} = \varphi
  }
  ...
  $\forall c \in C' \exists \varphi_c \in Q \qty(c \in \dom(\varphi_c) \cup B_2)$であり、\thmref{有限全順序集合の最大元}より、$\qty{\varphi_c \mid c \in C'} \subseteq Q$は最大元$\varphi_q$を持つ。

  ここで、$\Im(\varphi_q) \subseteq \Im(q)$であるので、$C' \subseteq C(\varphi_q)$であり、$C(\varphi_q)$が一次独立であることに反する。
...
%   \thmref{well-definedな写像}より定める演算$+'$および環準同型$\rho'$が存在する。
...
\lem*{
  ...
  $\bigcup \qty{B_1, B_2, B_3, B_4} \subseteq \bigoplus A$より明らか。
}
