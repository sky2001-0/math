...
\cor*{
  ...
  以下を満たす$\Eq(f, g), u$は、$f, g$の等価子である。
  \eqg*{
    \Eq(f, g) \coloneqq \qty{x \in X \mid f(x) = g(x)} \\*
    u(\dom(f)) \coloneqq \iota_{\Eq(f, g), \dom(f)}
  }
}

\dfn{小さい圏}{
  圏$\bm{C}$について、集合$C$が存在して以下を満たすとき、$\bm{C}$を小さいと呼ぶ。
  \eqg*{
    \forall f \in \bm{C} \qty(f \in \mathbf{Set}) \\*
    \forall c \in \Obj_{\bm{C}} \qty(c \in C)
  }
}
