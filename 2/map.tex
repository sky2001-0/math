...
\rem{写像は射}{
  ...
  ここで、記号の濫用であるが、$\id_X$をして$X$を表し、$X$をして$\id_X$を表す。
}
...
\dfn{写像の全体}{
  記号の濫用であるが、$X$から$Y$への写像全体がなす集合を、$\Hom_{\mathbf{Set}}(X, Y)$で表す。
  \eq*{
...
\dfn{空写像}{
  終域を$Y$としたとき、始域が空集合である写像は、一意に定まる。

  このような写像を空写像と呼ぶ。
}

\dfn{小さい圏}{
  圏$\bm{C}$について、以下を満たす圏$C$が存在して、$\bm{C}$と$C$が圏同型であるとき、$\bm{C}$を小さいと呼ぶ。
  \begin{enumerate}
    \item \dfnref{圏}を満たす写像からなる集合$C^\prime$が存在して、$f \in C \leftrightarrow f \in C^\prime$である。
  \end{enumerate}

  このとき、誤解のない範囲において、$C^\prime$を$\bm{C}$で表す。
}
...
\thm{集合と写像の圏は完備}{
  任意の小さい圏$\bm{J}$と、$\bm{J}$から$\mathbf{Set}$への任意の関手$X$を考える。

  このとき、$X$の極限が存在する。
}{
  $\forall j \in \Obj_{\bm{J}}$について、$X(j)$が恒等射すなわち集合とすることに注意して、
  集合$P$を以下で定める。
  \eq*{
    P \coloneqq \qty{\pi \in \Hom_{\mathbf{Set}}\qty(\Obj_{\bm{J}}, \bigcup \qty{X(j) \mid j \in \Obj_{\bm{J}}}) \mid \forall j \in \Obj_{\bm{J}} \qty(\pi(j) \in X(j))}
  }

  さらに、集合$L$を以下で定める。
  \eq*{
    L \coloneqq \qty{\pi \in P \mid \forall j, j^\prime \in \Obj_{\bm{J}} \forall i \in \Hom_{\bm{J}}(j, j^\prime) \qty\big(X(i)(\pi(j)) = \pi(j^\prime))}
  }

  $\mathbf{Set}$から$\Func(\Obj_{\bm{J}}, \mathbf{Set})$への対角関手$\Delta$について、
  以下で定める関数記号$u$を考える。
  \eq*{
    u(j)(\pi) \coloneqq \pi(j)
  }

  $u$は、$\bm{J}$の射$i \colon j \to j^\prime$について$\forall \pi \in L \qty(\qty(X(i) \circ u(j))(\pi) = X(i)(u(j)(\pi)) = X(i)(\pi(j)) = \pi(j^\prime) = u(j^\prime)(\pi))$を満たすので、
  $\Delta(L)$から$X$への自然変換である。

  \vskip\baselineskip

  このとき、$L, u$が極限となることを示す。

  集合$Y$と、$\Delta(Y)$から$X$への自然変換$f$を考える。

  このとき、写像$g \colon Y \to P$を以下で定める。
  \eq*{
    g(y)(j) \coloneqq f(j)(y)
  }

  $f$が自然変換であるので、$\forall y \in Y \qty(g(y) \in L)$となる。
  ゆえに、$g \colon Y \to L$として再定義する。

  写像$f$を、$x \mapsto f(x)$と表す。

  $\bm{J}$の任意の対象$j$について、$\qty(u(j) \circ g)(y) = u(j)(g(y)) = g(y)(j) = f(j)(y)$である。

  よって、$f = u \circ \Delta(g)$が成り立つ。

  \vskip\baselineskip

  $g^\prime \colon Y \to L$が存在して、$f = u \circ \Delta(g^\prime)$が成り立つとする。

  $g \neq g^\prime$とする。
  写像の相等より$\exists y \in Y \qty(g(y) \neq g^\prime(y))$であり、
  再び写像の相等より$\exists j \in \Obj_{\bm{J}} \exists y \in Y \qty(g(y)(j) \neq g^\prime(y)(j))$である。

  $g$の定義より、$\exists j \in \Obj_{\bm{J}} \exists y \in Y \qty(g^\prime(y)(j) \neq f(j)(y))$である。

  ゆえに、$\qty(u(j) \circ g^\prime)(y) = u(j)(g^\prime(y)) = g^\prime(y)(j) \neq f(j)(y)$であり、$f \neq u \circ \Delta(g^\prime)$となって反する。
...
%   集合$P$を以下で定める。
%   \eq*{
%     P \coloneqq \qty{\qty(j, x) \in \Obj_{\bm{J}} \times \bigcup \qty{X(j) \mid j \in \Obj_{\bm{J}}} \mid x \in X(j)}
%   }
...
\thm{非空からなる積は非空}{
  ...
  写像$g \colon J \to \bigcup Z$を$g(j) \coloneqq f(X(j))$で定義すると、$\forall j \in J \qty(f(j) \in X(j))$であるので、$f \in \prod X$である。
