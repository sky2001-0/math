...
\dfn{写像の全体}{
  ...
  \eq*{
    Y^X \coloneqq \qty{\text{$\qty(\qty(X, Y), G)$の特徴づける写像} \mid G \in \P(X \times Y) \land \text{$G$の特徴づける関係は、右一意的かつ$X$について左全域的}}
  }
}
...
\dfn{恒等写像}{
  ...
  $\qty(\qty(X, X), \Delta)$により特徴づけられる写像$\id{X}$を恒等写像と呼ぶ。
  \eq*{
...
\thm{選択公理が与える写像}{
  ...
  $A$の特徴づける関係は、仮定より$X$について左全域的で、$A$の定義より右一意的である。
...
\thm{像の性質}{
  ...
  \eqg*{
    ...
    \forall B \in \P(\P(X)) \qty(f \qty(\bigcup B) = \bigcup \qty{f(A) \mid A \in B}) \\*
    \forall B \in \P(\P(X)) \qty(f \qty(\bigcap B) \subset \bigcap \qty{f(A) \mid A \in B}) \\*
...
\thm{原像の性質}{
  ...
  \eqg*{
    ...
    \forall B \in \P(\P(Y)) \qty(f^{-1}(\bigcup B) = \bigcup \qty{f^{-1}(A) \mid A \in B}) \\*
    \forall B \in \P(\P(Y)) \qty(f^{-1}(\bigcap B) = \bigcap \qty{f^{-1}(A) \mid A \in B}) \\*
