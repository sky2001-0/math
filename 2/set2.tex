...
\lem{有限集合の部分}{
  ...
  単射$f \in X^Y, f(y) = y$と、$n \in \N$で全単射$g \in n^X$が存在する。

  ゆえに、単射$g \circ f \in n^Y$が存在して、\lemref{単射と有限}より成り立つ。
}

\lem{有限集合の像}{
  有限集合$X$、集合$Y$、写像$f \in Y^X$について、像$f(X)$は有限であり、$\abs{f(X)} \leq \abs{X}$である。
}{
  \thmref{全射と右逆写像}より、$f \in f(X)^X$は右逆写像すなわち単射$f^{-1} \in X^{f(X)}$を持つ。

  $n \in \N$と全単射$g \in n^X$が存在するので、単射$g \circ f^{-1} \in n^{f(X)}$が存在して、\lemref{単射と有限}より成り立つ。
}
...
\lem*{
  集合$X$について、自然な全単射$\sigma \in \qty(2^X)^{\P(X)}$が存在する。
}{
...
\lem{自然数から集合への写像の全体}{
  集合$X$と$n \in \N_{\geq 1}$について、全単射$\sigma \in \qty(X^{s(n)})^{X^n \times X}$が存在する。

  また、全単射$\sigma' \in \qty(X^1)^X$が存在する。
}{
...
\thm*{
  集合$X, Y, Z$について、自然な全単射$\sigma \in \qty(X \times \qty(Y \times Z))^\qty(\qty(X \times Y) \times Z)$が存在する。
}{
