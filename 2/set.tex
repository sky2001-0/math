...
\axi{置換の公理図式}{
  アリティ\num{2}の述語$\psi$をパラメータとする以下の公理図式を考える。
  \eq*{
...
\thm{分出の公理図式}{
  ...
  排中律より、
  \eq*{
    \forall X \qty\Big(\exists a \qty(a \in X \land P(a)) \lor \forall a \lnot \qty(a \in X \land P(a)))
  }
  ...
  $\psi(x, y) \land \psi(x, z)$とすると、$P(x)$であるとき$y = x = z$であり、$\lnot P(x)$であるとき$y = a = z$である。

  排中律より、$y = z$が成り立つ。

  したがって\axiref{置換の公理図式}より、
  \eq*{
