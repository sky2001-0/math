...
\dfn{完全}{
  集合$X$上の関係$\mathfrak{R}$が完全であるとは、以下を満たすことである。
  \eq*{
    \forall x, y \in X \qty(x \neq y \rightarrow \mathfrak{R}(x, y) \lor \mathfrak{R}(y, x))
  }
}
...
\dfn{写像が誘導する自己関係}{
  写像$f \colon X \to Y$と、$Y$上の自己関係$\mathfrak{R}_Y$を考える。

  このとき、以下で定める$X$上の自己関係$\mathfrak{R}_X$を考える。
  \eq*{
    \mathfrak{R}_X(x, y) \defiff \mathfrak{R}_Y(f(x), f(y))
  }

  この$\mathfrak{R}_X$を、$f$が$\mathfrak{R}_Y$から誘導する自己関係と呼ぶ。
}

\rem{包含写像が誘導する自己関係}{
  集合$X$上の自己関係$\mathfrak{R}$について、特に明示しなければ$X$の部分$A$の自己関係を、包含写像が誘導する自己関係で定めるものとする。
}

\cor{写像が誘導する自己関係}{
  写像$f \colon X \to Y$と、$Y$上の自己関係$\mathfrak{R}_Y$を考える。

  $f$が$\mathfrak{R}_Y$から誘導する自己関係$\mathfrak{R}_X$を考える。

  このとき、以下が成り立つ。
  \begin{enumerate}
    \item $\mathfrak{R}_Y$が反射的ならば、$\mathfrak{R}_X$は反射的である。
    \item $\mathfrak{R}_Y$が対称的ならば、$\mathfrak{R}_X$は対称的である。
    \item $f$が単射かつ$\mathfrak{R}_Y$が反対称的ならば、$\mathfrak{R}_X$は反対称的である。
    \item $\mathfrak{R}_Y$が推移的ならば、$\mathfrak{R}_X$は推移的である。
    \item $\mathfrak{R}_Y$が完全ならば、$\mathfrak{R}_X$は完全である。
  \end{enumerate}
}
...
\dfn{前順序が誘導する圏}{
  前順序集合$\qty(X, \preccurlyeq), \preccurlyeq = \qty(\qty(X, X), G)$について、$G$の元$\qty(x, y)$を考える。
...
\dfn{写像が誘導する同値関係}{
  写像$f \colon X \to Y$について、$f$が$=_Y$から誘導する自己関係は同値関係である。

  この同値関係を、写像$f$が誘導する同値関係と呼ぶ。
}

\cor*{
  写像$f \colon X \to Y$について、$f$が誘導する同値関係$\sim_f$を考える。

  $\sim_f = \qty(\qty(X, X), G)$について、$G, \qty(\pi_0, \pi_1)$は$f$の核対である。
...
\lem{集合の圏における余等化子の存在}{
  ...
  以下の集合$G$を考える。
  \eq*{
    G \coloneqq \bigcap \qty{G^\prime \in Y \times Y \mid \Im(q) \subset G^\prime \land \text{$\qty(\qty(Y, Y), G^\prime)$は同値関係}}
  }

  共通部分をとられる集合は空ではないので、$\sim \coloneqq \qty(\qty(Y, Y), G)$は同値関係となる。
  ...
  $h$が誘導する同値関係$\sim_h = \qty(\qty(Y, Y), S)$について、$\forall x_0, x_1 \in X \qty(\qty(f(x_0), g(x_1)) \in S)$である。

  すなわち$\Im(q) \subset S$であり、$G$の定義より$G \subset S$である。
...
\cor{商集合は余等化子}{
  集合$X$上の同値関係$\sim = \qty(\qty(X, X), G)$、$\sim$の商写像$u$、積$X \times X, \pi$を考える。
...
\thm{写像の標準分解}{
  ...
  写像$f$が誘導する同値関係$\sim_f = \qty(\qty(X, X), G)$とする。

  このとき、$G, \qty(\pi_0, \pi_1)$は$f$の核対であり、$X / \sim_f, u$は$\pi_0, \pi_1$の余等化子である。
...
\begin{center}
  \begin{minipage}{0.4\linewidth}
    \begin{tikzcd}
      G \ar[r, "\pi_0"] \ar[d, "\pi_1"'] & X \ar[d, "u"] \ar[ddr, "f", bend left] & \\
