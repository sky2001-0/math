...
\cor*{
  $\varnothing$上の関係は一意に定まり、それは、反射的、対称的、反対称的、推移的である。
}

\dfn{完全}{
  集合$X$上の関係$\mathfrak{R}$が完全であるとは、以下を満たすことである。
  \eq*{
    \forall x, y \in X \qty(\mathfrak{R}(x, y) \lor \mathfrak{R}(y, x))
  }
}
...
\thm{well-definedな写像}{
  空でない集合$X$上の同値関係$\sim_{X}$、集合$Y$上の同値関係$\sim_{Y}$について、写像$f \colon X \to Y$を考える。

  $f$が$\sim_{X}, \sim_{Y}$と両立することは、写像$h \colon X / \sim_{X} \to Y / \sim_{Y}$が存在して以下を満たすことと必要十分である。
  \eq*{
...
\thm{標準分解}{
  ...
  \thmref{well-definedな写像}より、写像$\bar{f} \colon X / \sim_{f} \to Y$が存在して、$\Im(\bar{f}) = \Im(f)$である。
