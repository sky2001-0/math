...
\lem{集合の圏における余等化子の存在}{
  ...
  以下の集合$\sim$を考える。
  \eq*{
    \sim \coloneqq \bigcap \qty{G^\prime \in Y \times Y \mid \Im(q) \subseteq G^\prime \land \text{$G^\prime$は$Y$上の同値関係}}
  }
  ...
  すなわち$\Im(q) \subseteq \sim_f$であり、$G$の定義より$G \subseteq \sim_f$である。
