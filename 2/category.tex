...
\dfn{自然変換の水平合成}{
  ...
  $F$から$F^\prime$への自然変換$\eta$と、$G$から$G^\prime$への自然変換$\theta$について、以下で定義するアリティ\num{1}の関数記号$\theta * \eta$は$G \circ F$から$G^\prime \circ F^\prime$への自然変換である。
  \eq*{
    \qty(\theta * \eta)(a) \coloneqq \theta(F^\prime(a)) \circ G(\eta(a))
  }

  $\theta * \eta$を、$\eta$と$\theta$の水平合成と呼ぶ。
}
...
\begin{center}
  ...
  \begin{minipage}{0.5\linewidth}
    ...
    \begin{tikzcd}
      ...
      G(F(a)) \ar[rr, crossing over, "G(F(f))"] \ar[dd, "G(\eta(a))"'] \ar[ur, "\theta(F(a))"] \ar[rd, "\qty(\theta * \eta)(a)", red] & & G(F(b)) \ar[ur, "\theta(F(b))"'] \ar[rd, "\qty(\theta * \eta)(b)", red] \\
      & G^\prime(F^\prime(a)) \ar[rr] & & G^\prime(F^\prime(b)) \\
      G(F^\prime(a)) \ar[rr, "G(F^\prime(f))"'] \ar[ur, "\theta(F^\prime(a))"] & & G(F^\prime(b)) \ar[from=uu, crossing over, "G(\eta(b))"] \ar[ur, "\theta(F^\prime(b))"'] \\
    \end{tikzcd}
  \end{minipage}
\end{center}
...
\thm{相互交換法則}{
  ...
  \eq*{
    \qty(\theta \circ \eta) * \qty(\tau \circ \sigma) = \qty(\theta * \tau) \circ \qty(\eta * \sigma)
  }
}{
  $\bm{C}$の任意の対象$a$について、以下が成り立つ。
  \eqa*{
    \qty(\qty(\theta \circ \eta) * \qty(\tau \circ \sigma))(a)
    ...
    &= \qty(\theta * \tau)(a) \circ \qty(\eta * \sigma)(a) \\*
    &= \qty(\qty(\theta * \tau) \circ \qty(\eta * \sigma))(a)
  }
...
\begin{center}
  \begin{minipage}{0.5\linewidth}
    $\bm{E}$ \\
    \begin{tikzcd}
      G(F(a)) \ar[r, "\eta(F(a))"] \ar[d, "G(\sigma(a))"'] \ar[dd, bend right=90, red, "G\qty((\tau \circ \sigma)(a))"'] \ar[rd, red, "(\eta * \sigma) (a)"] & G^\prime(F(a)) \ar[r, "\theta(F(a))"] \ar[d, "G^\prime(\sigma(a))"] & G^{\prime \prime}(F(a)) \ar[d, "G^{\prime \prime}(\sigma(a))"] \\
      G(F^\prime(a)) \ar[r, "\eta(F^\prime(a))"] \ar[d, "G(\tau(a))"'] & G^\prime(F^\prime(a)) \ar[r, "\theta(F^\prime(a))"] \ar[d, "G^\prime(\tau(a))"] \ar[rd, red, "(\theta * \tau) (a)"] & G^{\prime \prime}(F^\prime(a)) \ar[d, "G^{\prime \prime}(\tau(a))"] \\
      G(F^{\prime \prime}(a)) \ar[r, "\eta(F^{\prime \prime}(a))"'] \ar[rr, bend right, red, "\qty(\theta \circ \eta) \qty(F^{\prime \prime} (a))"'] & G^\prime(F^{\prime \prime}(a)) \ar[r, "\theta(F^{\prime \prime}(a))"'] & G^{\prime \prime}(F^{\prime \prime}(a)) \\
    \end{tikzcd}
  \end{minipage}
\end{center}
...
\dfnf{圏$\bm{\rightrightarrows}$}{並行射からなる圏}{
  以下を満たす相異なる\num{4}つの射$a, b, f, g$からなる圏を、$\bm{\rightrightarrows}$で表す。
  \eqg*{
    a = \dom(a) \land b = \dom(b) \\*
    a = \dom(f) \land b = \cod(f) \\*
    a = \dom(g) \land b = \cod(g)
  }
}

\dfnf{圏$\bm{\rightarrow \leftarrow}$}{並行射からなる圏}{
  以下を満たす相異なる\num{5}つの射$a, b, c, f, g$からなる圏を、$\bm{\rightarrow \leftarrow}$で表す。
  \eqg*{
    a = \dom(a) \land b = \dom(b) \land c = \dom(c) \\*
    a = \dom(f) \land c = \cod(f) \\*
    b = \dom(g) \land c = \cod(g)
  }
}

\rem{集合は離散圏}{
  集合$A$について、その要素を対象とみなすと、集合$A$の要素であることは圏となる。
}

\dfn{小さい圏}{
  圏$\bm{C}$について、集合$C$が存在して、以下を満たすとき、$\bm{C}$は小さいと呼ぶ。
  \eq*{
    \forall c \qty(c \in \bm{C} \leftrightarrow c \in C)
  }
}
...
\dfn{普遍射}{
  ...
  \eqg*{
    \dom(u) = F(x) \land \cod(u) = a \\*
    \forall y \in \Obj_{\bm{D}} \forall f \in \bm{C} \qty\Big(\dom(f) = F(y) \land \cod(f) = a \rightarrow \exists! g \in \bm{D} \qty\big(\dom(g) = y \land \cod(g) = x \land f = u \circ F(g)))
  }
}
...
\lem{普遍射の一意性}{
  ...
  このとき、以下を満たす。
  \eq*{
    \exists! h \in \bm{D} \qty(\dom(h) = x^\prime \land \cod(h) = x \land u^\prime = u \circ F(h))
  }

  さらに上で与える$h$は同型射であり、その逆射$h^{-1}$は$u = u^\prime \circ F(h^{-1})$を満たす。
}{
  $x, u$は普遍射であるので、$x^\prime, u^\prime$について定義より、
  $\bm{D}$の射$g$であって$\dom(g) = x^\prime \land \cod(g) = x \land u^\prime = u \circ F(g)$なる射が一意に存在する。

  同様に$x^\prime, u^\prime$は普遍射であるので、$x, u$について定義より、
  $\bm{D}$の射$g^\prime$であって$\dom(g^\prime) = x \land \cod(g^\prime) = x^\prime \land u = u^\prime \circ F(g^\prime)$なる射が一意に存在する。

  ゆえに、$u = u^\prime \circ F(g^\prime) = u \circ F(g) \circ F(g^\prime) = u \circ F(g \circ g^\prime)$が成り立つ。

  $x, u$は普遍射であるので、$x, u$について定義より、
  $\bm{D}$の射$i$であって$\dom(i) = x \land \cod(i) = x \land u = u \circ F(i)$なる射が一意に存在する。
...
\begin{center}
  ...
  \begin{minipage}{0.25\linewidth}
    ...
    \begin{tikzcd}
      \Delta(Y) \ar[r, "\Delta(q)", dotted] \ar[dr, "p"'] & \Delta\qty(\Eq(X(f), X(g))) \ar[d, "u"] \\
      & X \\
    \end{tikzcd}
  \end{minipage}
\end{center}
...
\dfn{和}{
  離散圏$\bm{J}$、圏$\bm{C}$と、$\bm{C}$から$\Func(\bm{J}, \bm{C})$への対角関手$\Delta$を考える。

  $\bm{J}$から$\bm{C}$への関手$X$について、$X$から$\Delta$への余普遍射を$X$の和と呼ぶ。
}
...
\rem{左右の随伴の存在}{
  左随伴の存在から右随伴の存在を導くには、（もちろん右から左も同様であるが、）圏論的な選択公理を導入しなければならない。

  これを回避するために、そもそも左右の随伴が存在することを前提にする立場をとる。
}
...
\dfn{随伴}{
  ...
  \eqg*{
    F = \qty(\theta * F) \circ \qty(F * \eta) \\*
    G = \qty(G * \theta) \circ \qty(\eta * G)
  }
...
\begin{center}
  \begin{minipage}{0.3\linewidth}
    $\Func(\bm{D}, \bm{C})$ \\
    \begin{tikzcd}
      F \ar[rr, "F * \eta"] \ar[drr, "F"'] & & F \circ G \circ F \ar[d, "\theta * F"] \\
      & & F
    \end{tikzcd}
  \end{minipage}
  \begin{minipage}{0.3\linewidth}
    $\Func(\bm{C}, \bm{D})$ \\
    \begin{tikzcd}
      G \ar[rr, "\eta * G"] \ar[drr, "G"'] & & G \circ F \circ G \ar[d, "G * \theta"] \\
      & & G
    \end{tikzcd}
  \end{minipage}
\end{center}
...
\lem{随伴は随伴関手}{
  ...
  $\bm{C}$の任意の対象$a$について、$G(a), \eta(a)$が$F$から$a$への普遍射となることを示す。

  ???
}

\lem{左随伴は余極限を保存する}{
  圏$\bm{J}, \bm{C}, \bm{D}$と、$\bm{J}$から$\bm{D}$への関手$X$、$\bm{D}$から$\bm{C}$への左随伴関手$F$を考える。

  $\qty(x, u)$が$X$の余極限であるならば、$\qty(F(x), F * u)$は$F \circ X$の余極限である。
}{
  $F$は左随伴であるため、$\bm{C}$の対象$c$について、$\qty(G(c), \epsilon(c))$は普遍射である。
}
