...
\dfn{射}{
  圏論では、\num{5}つの公理（
    \axiref{始域と終域の冪等性}、\axiref{合成}、\axiref{合成射の始域と終域}、\axiref{合成射の結合}、\axiref{対象は恒等射}
  ）を与える。
...
\axi{始域と終域の冪等性}{
  \eqg*{
    ...
    \forall f \qty(\cod(\dom(f)) = \dom(f)) \\*
    \forall f \qty(\dom(\cod(f)) = \cod(f))
  }
}

\dfn{合成}{
  アリティ\num{3}の述語記号$\Comp()$を考える。
}
...
\dfn{合成}{
  $\Comp(f, g, h)$は、述語$\cod(f) = \dom(g)$上の関数グラフである。
...
\axi{合成射の始域と終域}{
  \eq*{
    \forall f, g, h \qty(\cod(f) = \dom(g) \rightarrow \qty(\dom(g \circ f) = \dom(f) \land \cod(g \circ f) = \cod(g)))
  }
}
