...
\dfn{極限}{
  ...
  $\bm{D}$から$\bm{C}$への関手$X$について、$\Delta$から$X$への普遍射$x, u$を$X$の極限と呼ぶ。
}
...
\dfn{引き戻し}{
  ...
  以下で定める類$\bm{\rightarrow \leftarrow}$は、圏である。
  \eq*{
    h \in \bm{\rightarrow \leftarrow} \defiff h = f \lor h = g \lor h = \dom(f) \lor h = \dom(g) \lor h = \cod(f)
  }

  圏$\bm{\rightarrow \leftarrow}$から$\bm{C}$への以下で定める関手$\id$を考える。
  \eq*{
    \id(h) \coloneqq h
  }

  このとき、$\id$の極限を、$f, g$の引き戻しと呼ぶ。
}
...
\begin{center}
  ...
  \begin{minipage}{0.4\linewidth}
    ...
    \begin{tikzcd}
      ...
      & & & \\
      & x \ar[ld, "u(a)"'] \ar[rr, "u(b)"] \ar[rd, "u(c)"'] & & b \ar[ld, "g"] \\
      a \ar[rr, "f"'] & & c & \\
    \end{tikzcd}
  \end{minipage}
  \begin{minipage}{0.2\linewidth}
...
\dfn{等化子}{
  ...
  以下で定める類$\bm{\rightrightarrows}$は、圏である。
  \eq*{
    h \in \bm{\rightrightarrows} \defiff h = f \lor h = g \lor h = \dom(f) \lor h = \cod(f)
  }

  圏$\bm{\rightrightarrows}$から$\bm{C}$への以下で定める関手$\id$を考える。
  \eq*{
    \id(h) \coloneqq h
  }

  このとき、$\id$の極限を、$f, g$の等価子と呼ぶ。
}
...
\begin{center}
  ...
  \begin{minipage}{0.45\linewidth}
    ...
    \begin{tikzcd}
      ...
      & & & \\
      & \Eq(f, g) \ar[ld, "u(a)"'] \ar[rr, "u(a)"] \ar[rd, "u(b)"'] & & a \ar[ld, "g"] \\
      a \ar[rr, "f"'] & & b & \\
    \end{tikzcd}
  \end{minipage}
  \begin{minipage}{0.25\linewidth}
...
\thm{極限の存在定理}{
  ...
  \begin{enumerate}
    ...
    \item $\bm{C}$の任意の射$f, g$は、等化子を持つ。
  \end{enumerate}

  このとき、$\bm{J}$から$\bm{C}$への関手$X$は極限を持つ。
}{
  $\hat{\bm{J}}$から$\Obj_{\bm{J}}$への関手$\cod$、$\cod(\hat{f}) = \cod(f)$と、$\Obj_{\bm{J}}$から$\bm{J}$への自明な関手$I$を考える。

  $P = X \circ I$と、$R = X \circ I \circ \cod$を考える。
  ...
  $\lambda \coloneqq \pi \circ \Delta_2(u(\prod P))$が、$\Delta_2(\Eq(\bar{s}, \bar{t}))$から$X$への自然変換であることを示す。

  $\bm{J}$の射$j$について、等化子と積の定義より以下が成り立つ。
  \eqa*{
    X(j) \circ \lambda(\dom(j))
    &= X(j) \circ \pi(\dom(j)) \circ \Delta_2(u(\prod P))(\dom(j)) \\*
    &= X(j) \circ \pi(\dom(j)) \circ u(\prod P) \\*
    &= t(j) \circ u(\prod P) \\*
    &= r(j) \circ \bar{t} \circ u(\prod P) \\*
    &= r(j) \circ \bar{s} \circ u(\prod P) \\*
    &= s(j) \circ u(\prod P) \\*
    &= \pi(\cod(j)) \circ u(\prod P) \\*
    &= \lambda(\cod(j))
  }
  ...
  等化子より、一意な$\bm{C}$の射$l \colon y \to \Eq(\bar{s}, \bar{t})$が存在して、$w = u(\prod P) \circ l$である。
...
\begin{center}
  ...
  \begin{minipage}{0.5\linewidth}
    $\bm{C}$ \\
    \begin{tikzcd}
      \Eq(\bar{s}, \bar{t}) \ar[rr, "u(\prod P)"] \ar[dd, "u(\prod P)"'] \ar[rrdd, "u(\prod R)"] & & \prod P \ar[dd, "\bar{t}"] \ar[rd, "\pi(\dom(j))"] \ar[rddd, "t(j)"'] & \\
      & & & X(\dom(j)) \ar[dd, "X(j)"] \\
...
\dfn{押し出し}{
  ...
  このとき、$f^\opp, g^\opp$の引き戻しを、$f, g$の押し出しと呼ぶ。
}
...
\dfn{余等化子}{
  ...
  このとき、$f^\opp, g^\opp$の等化子を、$f, g$の余等化子と呼ぶ。
}
