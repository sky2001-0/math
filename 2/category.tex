...
\dfn{射}{
  圏論では、\num{6}つの公理（
    \axiref{始域と終域の冪等性}、\axiref{合成}、\axiref{始域と終域}、\axiref{合成射の始域と終域}、\axiref{合成射の結合}、\axiref{対象は恒等射}
  ）が与えられる。
...
\axi{合成射の始域と終域}{
  \eq*{
    \forall f, g, h \qty(\Comp(f, g, h) \rightarrow \cod(f) = \dom(g) \land \dom(h) = \dom(f) \land \cod(h) = \cod(g))
  }
}

\dfn{合成}{
  $\Comp(f, g, h)$であるとき、簡単のために$h$を$g \circ f$で表す。
}
