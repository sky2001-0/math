...
\lem*{
 \eq*{
   \forall A \qty(A \neq \varnothing \land \forall X \in A \qty(M(X)) \rightarrow M \qty(\bigcap A))
 }
}{
  まず、以下が成り立つ。
  \eq*{
...
\lem*{
  ...
  \eqg*{
    ...
    \forall Z \qty(Z \cap Y \in \qty{Z \in \P(X \cap Y) \mid M(Z)})
  }
  ゆえに、
  \eqg*{
...
\thm{Peanoの公理}{
  ...
  定義より、$\N = \bigcap \qty{Z \in \P(E) \mid M(Z)}$
...
\thm{数学的帰納法}{
  アリティ1の述語記号$P$について、
  \eq*{
    \forall n \qty(n \in \N \rightarrow P(n)) \leftrightarrow P(0) \land \forall n \qty(n \in \N \land \qty(P(n) \rightarrow P(s(n))))
  }
}{
  右は自明。\\*
