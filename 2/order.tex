...
\cor*{
  集合$P$について、$\qty(P, \subseteq)$は半順序集合である。また、$\qty(P, \supseteq)$も半順序集合である。
}
...
\cor*{
  集合$X$について、$\qty(\P(X), \subseteq)$は完備束である。
}
...
\dfn{区間}{
  全順序集合$P$と$a, b \in P$について、以下で定める$P$の部分集合をそれぞれ、開区間$\outsqbr{a, b}$、閉区間$\qty[a, b]$と呼ぶ。
  \eqg*{
    \outsqbr{a, b} \coloneqq \qty{x \in P \mid a < x \land x < b} \\*
...
\dfn{順序数}{
  ...
  \eq*{
    \forall x \in \alpha \qty(x \subseteq \alpha)
  }
...
\lem*{
  ...
  $\min \alpha \subseteq \alpha$であるので、$\beta \in \alpha$より最小性に反する。
}
...
\lem{順序数の所属と真包含の同値性}{
  ...
  順序数の定義より$\alpha \subseteq \beta$である。
  ...
  任意の$x \in \gamma$について、$\gamma \subseteq \beta$より$x \in \beta$である。

  $x \notin \alpha$とすると、$x \in \beta \setminus \alpha$より、$\gamma = x \lor \gamma \in x$であり、\lemref{循環所属の禁止}に反する。

  ゆえに$x \in \alpha$であるので、$\gamma \subseteq \alpha$である。

  \vskip\baselineskip

  任意の$y \in \alpha$について、$\alpha \subseteq \beta$より$y \in \beta$であるため、$\beta$が全順序より$y \in \gamma \lor y = \gamma \lor \gamma \in y$である。

  $y = \gamma$のとき、$\gamma \in \alpha$であるが$\gamma$の定義に反する。

  $\gamma \in y$ならば、$y \in \alpha$より、$\gamma \in \alpha$となり、同様に反する。

  ゆえに$y \in \gamma$であるので、$\alpha \subseteq \gamma$である。
...
\lem{順序数の比較可能性}{
  ...
  \lemref{順序数の所属と真包含の同値性}より特に以下が成り立つ。
  \eq*{
  ...
  \lemref{順序数の所属と真包含の同値性}より、$\alpha \cap \beta \in \alpha \land \alpha \cap \beta \in \beta$である。
...
\lem{順序数からなる整列集合}{
  ...
  $b \cap B \neq \varnothing$のとき、$b \cap B \subseteq b$であり、$b$は整列集合であるため、$\min(b \cap B)$が存在する。
  これは、$B$の最小元である。
}
...
\lem{順序数の後続と比較}{
  ...
  \lemref{順序数の所属と真包含の同値性}より、$\beta \in \alpha$から$\beta \subsetneq \alpha$すなわち$\beta \subseteq \alpha \land \beta \neq \alpha$を得る。

  \lemref{順序数の比較可能性}より、$\alpha \subseteq \beta^+$を得る。
...
\lem{順序数の類別}{
  ...
  任意の$x \in \bigcup \alpha$について、$\exists \beta \in \alpha \qty(x \in \beta)$であり、$\beta \subseteq \alpha$から$x \in \alpha$である。

  ゆえに、$\bigcup \alpha \subseteq \alpha$である。

  \vskip\baselineskip

  任意の$x \in \alpha$について、\lemref{順序数の後続と比較}より$x^+ \in \alpha \lor x^+ = \alpha$であり、仮定より$x^+ \neq \alpha$であるので、$x^+ \in \alpha$である。

  $x \in x^+ \land x^+ \in \alpha$より、$\exists \beta \in \alpha \qty(x \in \beta)$すなわち$x \in \bigcup \alpha$である。

  したがって$\alpha \subseteq \bigcup \alpha$である。
}
...
\thm{超限帰納法}{
  ...
  $\min A \subseteq \alpha^+$と、最小性より$\forall \gamma \in \min A \qty(\psi(\gamma))$である。

  仮定より$\psi(\min A)$が成り立つが、$A$の定義より$\lnot \psi(\min A)$であり、反する。
}

\thm{無限降下法}{
  アリティ\num{1}の述語$\psi$について、以下が成り立つ。
  \eq*{
    \forall \alpha \qty\Big(\text{$\alpha$は順序数} \land \psi(\alpha) \rightarrow \exists \beta \in \alpha \qty(\psi(\beta))) \rightarrow \forall \alpha \qty\Big(\text{$\alpha$は順序数} \rightarrow \lnot \psi(\alpha))
  }
}{
  ある順序数$\alpha$が存在して、$\psi(\alpha)$であるとする。

  $A \coloneqq \qty{\beta \in \alpha^+ \mid \psi(\alpha)}$とすると、$\alpha \in A$より、$A$は空でない。

  $A$は順序数$\alpha^+$の部分であるので、最小元$\min A$が存在する。

  仮定より$\exists \beta \in \min A \qty(\psi(\beta))$であるので、$\beta \in A$である。

  $\beta \in \min A$より、$\min A$の最小性に反する。
...
\thm{超限再帰}{
  ...
  順序数$\beta \subseteq \alpha$について、以下を満たす写像$f_\beta \colon \beta \to A$の存在して、これが一意であることを示す。
  \eq*{
...
\lem{順序数の濃度の上限}{
  順序数$\alpha, \beta$について、以下が成り立つとき、単射$\alpha \to \beta$が存在する。
  \eq*{
    \forall \xi \in \alpha \qty(\text{全射$\xi \to \beta$が存在しない})
  }
}{
  $h \in \qty{\bigcup \beta^\xi \mid \xi \in \alpha}$について、$h$は全射でないので、$\beta \setminus \Im(h) \neq \varnothing$である。

  以下で与える写像$G \colon \qty{\bigcup \beta^\xi \mid \xi \in \alpha} \to \beta$を考える。
  \eq*{
    G(h) \coloneqq \min\qty(\beta \setminus \Im(h))
  }

  このとき、\thmref{超限再帰}より以下を満たす$f \colon \alpha \to \beta$が一意に定まる。
  \eq*{
    \forall \xi \in \alpha \qty(f(\xi) = G(f \circ \iota_{\xi, \alpha}))
  }
  ...
  $f(\eta) = G(f \circ \iota_{\eta, \alpha}) = \min\qty(\beta \setminus \Im(f \circ \iota_{\eta, \alpha})) \notin \Im(f \circ \iota_{\eta, \alpha})$である。

  $f(\xi) \in \Im(f \circ \iota_{\eta, \alpha})$より、$f(\xi) \neq f(\eta)$である。

  $\eta \in \xi$のときも同様である。

  \lemref{順序数の比較可能性}より、$\xi \neq \eta$のとき$\xi \in \eta \lor \eta \in \xi$であるので、$\xi \neq \eta$ならば$f(\xi) \neq f(\eta)$すなわち$f$は単射である。
}

\lem{整列集合の自己単調単射は増大}{
  整列集合$\qty(W, \leq)$と、狭義単調写像$f \colon W \to W$について、以下が成り立つ。
  \eq*{
    \forall x \in W \qty(x \leq f(x))
  }
}{
  $A \coloneqq \qty{x \in W \mid f(x) < x}$を考える。
...
\lem{切片からの狭義単調は存在しない}{
  空でない整列集合$\qty(W, \leq)$と$a \in W$について、狭義単調な$f \colon W \to W_{< a}$は存在しない。
}{
  存在するとする。

  $\iota_{W_{< a}, W}$は狭義単調より、$\iota_{W_{< a}, W} \circ f$は狭義単調である。

  \lemref{整列集合の自己単調単射は増大}より、$a \leq \qty(\iota_{W_{< a}, W} \circ f)(a)$である。

  $\iota_{W_{< a}, W}$は包含写像より、$\forall x \in W \qty(\iota_{W_{< a}, W}(x) < a)$より反する。
}
...
\lem{整列集合は順序型を持つ}{
  ...
  \lemref{順序数の比較可能性}、\lemref{切片からの狭義単調は存在しない}より、存在すれば一意である。
  ...
  一意性と\axiref{置換の公理図式}より、集合$A$が存在して、以下を満たす。
  \eq*{
...
\lem{Hartogsの補題}{
  ...
  $W \coloneqq \qty{\qty(S, R) \mid S \subseteq X \land R \subseteq S \times S \land \text{$\qty(S, R)$は整列集合}}$を考える。

  \lemref{整列集合は順序型を持つ}と\axiref{置換の公理図式}より、以下で定める集合$A$が存在する。
  \eq*{
  ...
  $\alpha \in A$を考えると、$S \subseteq X$について整列集合$\qty(S, R)$が存在して、順序同型$g \colon \qty(\alpha, \in) \to \qty(S, R)$が存在する。

  任意の$\beta \in \alpha$について、$g \circ \iota_{\beta, \alpha} \colon \beta \to \Im(g)$は順序同型であるので、$\beta \in A$である。
  ...
  順序数$\alpha$について、単射$f \colon \alpha \to X$が存在するとする。

  このとき、$f$の左逆写像$g$を用いて、$\Im(f)$上に以下の整列順序$\leq$を定義できる。
  \eq*{
    x < y \defiff g(x) \in g(y)
  }
...
\lem{Amannの不動点定理}{
  ...
  $x \in N$について、$f(x) \in f(N) \subseteq f(M) \subseteq M$である。
  ...
  $N$の空でない全順序部分$C$について、$N \subseteq M$より$\sup C \in M$が存在する。

  $C \subseteq N$より、任意の$y \in S$について、$\forall c \in C \qty(c \preccurlyeq y)$すなわち$y$は$C$の上界である。
...
\thm{Knaster-Tarskiの不動点定理}{
  ...
  任意の$a \in A$について、$\forall x \in P \qty(a \preccurlyeq x)$より、$a$は$P$の下界であるので、$a \preccurlyeq \inf P$である。
  ゆえに、$\inf P$は$A$の上界である。

  したがって$\sup A \preccurlyeq \inf P$であり、\lemref{単調像と上限}と単調性より$\sup A = \sup f(A) \preccurlyeq f(\sup A) \preccurlyeq f(\inf P)$である。

  よって、$f(\inf P)$は$A$の上界である。

  $\forall x \in P \qty(\inf f(P) \preccurlyeq f(x) \preccurlyeq x)$より$\inf f(P)$は$P$の下界であることと\lemref{単調像と下限}から、$f(\inf P) \preccurlyeq \inf f(P) \preccurlyeq \inf P$である。

  単調性より、$f(f(\inf P)) \preccurlyeq f(\inf P)$である。

  したがって、$f(\inf P) \in P$である。

  よって$\inf P \preccurlyeq f(\inf P)$を得るので、$\inf P = f(\inf P)$すなわち$\inf P \in S$である。

  \vskip\baselineskip

  すでに示した$\sup A \preccurlyeq \inf P$と$\sup A \preccurlyeq f(\inf P) = \inf P$より、$\sup A = \inf P \in S$である。
...
\thmf{Cantor-Schr\"{o}der-Bernsteinの定理}{Cantor-Schroder-Bernsteinの定理}{
  ...
  $X \subseteq Y \subseteq A$について、\lemref{像の性質}より$f(X) \subseteq f(Y)$であり、$B \setminus f(X) \supseteq B \setminus f(Y)$であり、
  \lemref{像の性質}より$g\qty(B \setminus f(X)) \supseteq g\qty(B \setminus f(Y))$であり、$F(X) \subseteq F(Y)$である。

  ゆえに、$F$は完備束$\qty(\P(A), \subseteq)$上の単調写像である。

  \thmref{Knaster-Tarskiの不動点定理}より、$\exists X_0 \subseteq A \qty(X_0 = F(X_0))$である。
...
\lem{冪集合の鎖の基本補題}{
  空でない半順序集合$\qty(P, \preccurlyeq)$について、以下で定める半順序集合$\qty(M, \subseteq)$を考える。
  \eq*{
    M \coloneqq \qty{C \subseteq P \mid C \neq \varnothing \land \text{$\qty(C, \preccurlyeq)$は全順序集合}}
  }
  ...
  $A$は全順序集合より、$C_x \subseteq C_y \lor C_y \subseteq C_x$である。

  $C_x \subseteq C_y$であるとき、$x, y \in C_y$より、$x \preccurlyeq y \lor y \preccurlyeq x$である。

  $C_y \subseteq C_x$であるときも同様である。

  \vskip\baselineskip

  $A \neq \varnothing$より$\bigcup A \neq \varnothing$である。

  $\bigcup A$は$A$の上界であり、$A$の任意の上界$U$について、$\forall C \in A \qty(C \subseteq U)$から$\bigcup A \subseteq U$であるため、$\bigcup A$は$A$の上限である。
}
...
\thm{Zornの補題}{
  ...
  \lemref{冪集合の鎖の基本補題}より、半順序集合$\qty(M, \subseteq)$は空でないかつ任意の空でない全順序部分は上限を持つ。
...
\thm{整列可能定理}{
  ...
  以下で定める集合$W$を考える。
  \eq*{
    W \coloneqq \qty{\qty(A, \leq) \mid A \subseteq X \land \text{$\qty(A, \leq)$は整列集合}}
  }

  $\qty(\varnothing, \varnothing) \in W$より、$W \neq \varnothing$である。

  $W$上の関係$\preccurlyeq$を以下で定める。
  \eq*{
    \qty(A_1, \leq_1) \preccurlyeq \qty(A_2, \leq_2) \defiff A_1 \subseteq A_2 \land \leq_1 = \leq_2 \cap \qty(A_1 \times A_1)
  }

  $W$の全順序部分$C$について、以下の組は整列集合であり、$C$の上界である。
  \eq*{
    \qty(\bigcup \qty{A \mid \qty(A, \leq) \in C}, \bigcup \qty{\leq \mid \qty(A, \leq) \in C})
  }

  \thmref{Zornの補題}より、極大元$\qty(A_0, \leq_0)$が存在する。

  $A_0 \subsetneq X$とすると、$\exists a \in X \qty(a \notin A_0)$であり、以下で定める組$G$が存在する。
  \eq*{
    G \coloneqq \qty(A_0 \cup \qty{a}, \leq_0 \cup \qty(A_0 \times \qty{a}))
  }

  $G \in W$であり、$\qty(A_0, \leq_0) \prec G$であるので、極大性に反する。
...
\thm{濃度の比較可能定理}{
  ...
  \lemref{順序数の比較可能性}より、$\alpha \subseteq \beta \lor \beta \subseteq \alpha$である。

  $\alpha \subseteq \beta$のとき、$\psi_b \circ \iota_{\alpha, \beta} \circ \psi_a^{-1}$は$A$から$B$への単射である。

  $\beta \subseteq \alpha$も同様である。
}
...
\dfn{細分}{
  半順序集合$P$のフィルター$F, G$に対して、$F \subseteq G$であるとき、$G$は$F$の細分であると呼ぶ。
}
...
\thm{超フィルターの存在}{
  ...
  $F$の細分の全体$\mathcal{F}$と半順序集合$\qty(\mathcal{F}, \subseteq)$を考える。
...
\dfn{集合におけるフィルター}{
  集合$X$について、半順序集合$\qty(\P(X) \setminus \qty{\varnothing}, \subseteq)$のフィルターを、集合$X$のフィルターと呼ぶ。
}
...
\thm{集合の超フィルター}{
  ...
  \begin{enumerate}
    ...
    \item $\forall A \subseteq X \qty(A \in \mathcal{F} \lor X \setminus A \in \mathcal{F})$
  \end{enumerate}
}{
  $1. \rightarrow 2.$を示す。
  ...
  $\mathcal{S} \coloneqq \qty{S \subseteq X \mid A \cup S \in \mathcal{F}}$について、定義より$\mathcal{F} \subseteq \mathcal{S} \land X \setminus A \in \mathcal{S}$である。
  ...
  $S \in \mathcal{S} \land T \subseteq X \land S \subseteq T$とすると、$A \cup S \in \mathcal{F} \rightarrow A \cup T \in \mathcal{F}$より$T \in \mathcal{S}$
  ...
  仮定より、$X \setminus A \in \mathcal{F} \subseteq \mathcal{F'}$であり、$\varnothing = A \cap \qty(X \setminus A) \in \mathcal{F'}$よりフィルターの定義に矛盾。背理法より示される。
}
...
\dfn{普遍}{
  ...
  \eq*{
    \forall A \subseteq X \exists \lambda_0 \in \Lambda \qty(\qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_0}} \subseteq A \lor \qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_0}} \subseteq X \setminus A)
  }
}
...
\thm{ネットの定めるフィルター}{
  ...
  \eq*{
    \mathcal{F} \coloneqq \qty{F \subseteq X \mid \exists \lambda_0 \in \Lambda \qty(\qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_0}} \subseteq F)}
  }
}{
  ...
  $F_1, F_2 \in \mathcal{F}$について、定義より$\exists \lambda_1, \lambda_2 \subseteq \Lambda \qty(\qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_1}} \subseteq F_1 \land \qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_2}} \subseteq F_2)$である。

  $\Lambda$が有向集合であることから、$\exists \lambda_3 \in \Lambda \qty(\lambda_1 \preccurlyeq \lambda_3 \land \lambda_2 \preccurlyeq \lambda_3)$であり、$\qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_3}} \subseteq F_1 \cap F_2$。

  ゆえに$F_1 \cap F_2 \in \mathcal{F}$である。

  \vskip\baselineskip

  $\forall F \in \mathcal{F} \forall G \subseteq \P(X)$について、$F \subseteq G$ならば定義より明らかに$G \in \mathcal{F}$
}
...
\lem{ネットとフィルターの基本補題}{
  ...
  今、$\qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_0}} \in \mathcal{F} \subseteq \mathcal{F'}$であるので、$\varnothing = F \cap \qty(x_\lambda)_{\lambda \in \Lambda_{\succcurlyeq \lambda_0}} \in \mathcal{F'}$より、フィルターの定義に矛盾。
...
\thm{フィルターの定める部分ネット}{
  ...
  $M$上の前順序$\forall \qty(\lambda_1, F_1), \qty(\lambda_2, F_2) \in M \qty(\qty(\lambda_1, F_1) \preccurlyeq \qty(\lambda_2, F_2) \defiff \lambda_1 \preccurlyeq \lambda_2 \land F_1 \supseteq F_2)$を考える。
  ...
  \lemref{ネットとフィルターの基本補題}より$\exists \lambda_4 \in \Lambda \qty(\lambda_3 \preccurlyeq \lambda_4 \land x_{\lambda_4} \in F_1 \cap F_2)$
  ...
  \lemref{ネットとフィルターの基本補題}より$\forall \lambda \in \Lambda \exists F \in \mathcal{F'} \exists \lambda_1 \in \Lambda \qty(\lambda \preccurlyeq \lambda_1 = \varphi(\lambda_1, F) \land \qty(\lambda_1, F) \in M)$である。

  したがって、$\qty(x_{\varphi(\mu)})_{\mu \in M}$は部分ネットである。

  \vskip\baselineskip

  今、\lemref{ネットとフィルターの基本補題}より$\forall F \in \mathcal{F'} \exists \lambda_0 \in \Lambda \qty(\qty(\lambda_0, F) \in M)$

  $\forall \qty(\lambda', F') \in M$について、$\qty(\lambda_0, F) \preccurlyeq \qty(\lambda', F')$ならば、$x_{\lambda'} \in F' \subseteq F$となる。

  よって、$F \in \qty{F \subseteq X \mid \exists \mu_0 \in M \qty(\qty(x_{\varphi(\mu)})_{\mu \in M_{\succcurlyeq \mu_0}} \subseteq F)}$
}
