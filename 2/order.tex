...
\dfn{単調}{
  半順序$\preccurlyeq$と両立する写像を単調である、または広義単調であると呼ぶ。

  特に、$\prec$と両立する写像を、狭義単調であると呼ぶ。
}
...
\dfn{上限}{
  ...
  $A$の上界全体の集合に最小元が存在するとき、これを$A$の上限と呼び、$\sup A$で表す。
}

\dfn{下限}{
  半順序集合$P$について、その部分集合$A$を考える。

  $A$の下界全体の集合に最大元が存在するとき、これを$A$の下限と呼び、$\inf A$で表す。
}
...
\lem{分配不等式}{
  束$P$について、以下が成り立つ。
  \eqg*{
    \sup{x, \inf{y, z}} \preccurlyeq \inf{\sup{x, y}, \sup{x, z}} \\*
    \inf{x, \sup{y, z}} \succcurlyeq \sup{\inf{x, y}, \inf{x, z}}
  }
}{
