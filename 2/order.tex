...
\lem{順序数の後者と比較}{
  ...
  \lemref{順序数の所属と真包含の同値性}より、$\beta \in \alpha$から$\beta \subsetneq \alpha$すなわち$\beta \subset \alpha \land \beta \neq \alpha$を得る。
...
\lem{順序数の類別}{
  ...
  任意の$x \in \alpha$について、\lemref{順序数の後者と比較}より$x^+ \in \alpha \lor x^+ = \alpha$であり、仮定より$x^+ \neq \alpha$であるので、$x^+ \in \alpha$である。
...
\lem{Hartogsの補題}{
  ...
  $\alpha \in A$を考えると、$S \subset X$について整列集合$\qty(S, R)$が存在して、順序同型$g \colon \qty(\alpha, \in) \to \qty(S, R)$が存在する。
...
\thm{Zornの補題}{
  ...
  \lemref{冪集合の鎖の基本補題}より、半順序集合$\qty(M, \subset)$は空でないかつ任意の空でない全順序部分は上限を持つ。
...
\thm{整列可能定理}{
  ...
  以下で定める集合$W$を考える。
  \eq*{
    W \coloneqq \qty{\qty(A, \leq) \mid A \subset X \land \text{$\qty(A, \leq)$は整列集合}}
  }
  ...
  \eq*{
    \qty(A_1, \leq_1) \preccurlyeq \qty(A_2, \leq_2) \defiff A_1 \subset A_2 \land \leq_1 = \leq_2 \cap \qty(A_1 \times A_1)
  }
