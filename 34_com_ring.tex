...
\lem{倍元と単項イデアル}{
  ...
  \begin{enumerate}
    \item $a$は$b$の倍元
    \item $a \in \eval{b}$
    \item $\eval{a} \subset \eval{b}$
  \end{enumerate}
}{
  $1. \rightarrow 2.$を示す。

  $\exists c \in R \qty(a = c b)$より、$a \in \eval{b}$である。\\*

  $2. \rightarrow 3.$を示す。

  $\forall a' \in \eval{a} \exists c' \in R \qty(a' = c' a)$である。
...
\dfn{単項イデアル}{
  可換環$R$と、元$a \in R$について、以下で定まる集合$\eval{a}$はイデアルである。
  \eq*{
    \eval{a} \coloneqq \qty{r a \mid r \in R}
  }
...
\cor*{
  ...
  \eq*{
    a \in I \rightarrow \eval{a} \subset I
  }
}
...
\dfn{素元}{
  可換環$R$と元$a \in R \setminus \qty{0_R}$について、単項イデアル$\eval{a}$が素イデアルとなるとき、$a$を素元と呼ぶ。
}

\lem{整域の素元は可逆でない既約元}{
  整域$D$について、素元は可逆でない既約元である。
}{
  素元$a$が可逆とすると、$1_D = a^{-1} a \in \eval{a}$であるので、$D = \eval{a}$より素イデアルではない。背理法より可逆でない。\\*

  素元$a$が既約元でないとする。$\exists x, y \in D \setminus D^\times \qty(x y = a)$

  $x y = a \in \eval{a}$であるので、素イデアルの定義から$\exists r \in D \qty(x = r a \lor y = r a)$である。
...
\thmf{PIDは\textit{Noether}}{PIDはNoether}{
  ...
  単項イデアル整域であるので、$\exists a \in D \qty(\eval{a} = J)$

  $a \in J$より、$\exists N \in \N \qty(a \in I_N)$より、$\eval{a} \subset I_N$

  $\forall N \in \N_{\geq N} \qty(\eval{a} \subset I_N \subset I_n \subset J = \eval{a})$
}

\thm{PIDの素イデアルは極大イデアル}{
  単項イデアル整域$D$の素イデアルは極大イデアルである。
}{
  極大イデアルでない素イデアル$\eval{a}$を考える。

  このとき$\exists b \in D \qty(\eval{a} \subsetneq \eval{b} \neq D)$

  $\exists c \in R \qty(a = c b)$である。$b \in \eval{a}$のとき、$\eval{a} = \eval{b}$となり仮定に反する。

  素イデアルの定義より$c \in \eval{a}$であるので、$\exists d \in R \qty(c = d a)$
...
\thm{PIDはUFD}{
  ...
  $1_R \in \eval{a}$とすると、$a$は可逆元となり、素元分解できる。
  ゆえに$\eval{a} \neq D$である。\\*

  \thmref{極大左イデアルの存在}、\thmref{極大イデアルは素イデアル}と$D$が単項イデアル整域であることより、素元$p_1$が存在して$a \in \eval{a} \subset \eval{p_1}$

  ゆえに$\exists a_1 \in D \qty(a = a_1 p_1)$と書ける。$a \neq 0_D$より$a_1 \neq 0_D$

  $\eval{a} \supset \eval{a_1}$とすると、$\exists b \in D \qty(a_1 = b a)$より$a_1 \qty(b p_1 - 1_R) = 0_R$であり、整域と$a_1 \neq 0_R$から$p_1 \in D^\times$

  これは\lemref{整域の素元は可逆でない既約元}に反する。ゆえに$\eval{a} \subsetneq \eval{a_1}$

  $a$は素元分解できないので、$a_1$も素元分解でもない。\\*

  帰納的にイデアル列$\eval{a} \subset \eval{a_1} \subset \cdots$がつくれるが、これは\thmref{PIDはNoether}に反する。
  ...
  $a \in \eval{p(n)}$より、素イデアルの定義から$\exists k \in n' \qty(p'(k) \in \eval{p(n)})$である。ゆえに$\eval{p'(k)} \subset \eval{p(n)}$

  \thmref{PIDの素イデアルは極大イデアル}より$\eval{p'(k)} = \eval{p(n)}$
