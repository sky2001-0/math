...
\dfn{群準同型}{
  群$G_1, G_2$について以下を満たすモノイド準同型$\varphi \colon G_1 \to G_2$が存在するとき、$\varphi$を群準同型写像、または単に群準同型と呼ぶ。
  \eq*{
...
\lem{マグマ準同型は群準同型}{
  群$G_1, G_2$について、マグマ準同型$\varphi \colon G_1 \to G_2$は群準同型である。
}{
  モノイド準同型であることを示す。
  \eqg*{
...
\thm{群準同型の単射性}{
  群準同型$\varphi \colon G_1 \to G_2$が単射であることは、以下と必要十分である。
  \eq*{
    \Ker(\varphi) = \qty{e}
  }
}{
  ...
  必要性を示す。
  $\forall x, y \in G_1 \land \varphi(x) = \varphi(y)$について、
  \eq*{
    e' = \varphi(e) = \varphi(x) \varphi(x^{-1}) = \varphi(y) \varphi(x^{-1}) = \varphi(y x^{-1})
  }

  ゆえに$x y^{-1} \in \Ker(\varphi)$で、$x = y$。
  すなわち単射。
}

\dfn{群同型}{
  群準同型$\varphi \colon G_1 \to G_2$が全単射であるとき、これを群同型写像、または単に群同型と呼ぶ。

  また、$G_1$から$G_2$への群同型写像が存在するとき、$G_1 \cong G_2$と表す。
}

\dfn{自己同型群}{
  マグマ$M$について、$\End(M)^\times$をは群をなす。
  これを自己同型群と呼び、$\Aut(M)$と表す。
}
...
\thm{可換群上の準同型全体}{
  ...
  写像$0_{\Hom(G, H)} \colon G \to H, 0_{\Hom(G, H)}(x) = 0_H$は群準同型であるので、単位元。

  写像$f_{\text{minus}} \colon G \to H, f_{\text{minus}}(x) = -f(x)$は群準同型であるので、逆元。
}
...
\dfn{群の作用}{
  群$\qty(G, \cdot)$について、$G$から、集合$X$上の全単射の全体への写像$\varphi \colon \qty(G, \cdot) \to \qty(\qty(X^X)^\times, \circ)$が群準同型であるとき、群作用、または単に作用と呼ぶ。
}
...
\lem{群作用の軌道分解}{
  ...
  $\forall w_x \in \varphi(G, x)$について$\exists g_x \in G \qty(w_x = g_x x)$であり、$w_x = \qty(g_x g) y \in \varphi(G, y)$であるので、$\varphi(G, x) \subseteq \varphi(G, y)$

  $\forall w_y \in \varphi(G, y)$について$\exists g_y \in G \qty(w_y = g_y y)$であり、$w_y = \qty(g_y g^{-1}) x \in \varphi(G, x)$であるので、$\varphi(G, y) \subseteq \varphi(G, x)$
}

\lem*{
  群$G$の$X$への群作用$\varphi$と、元$x \in X$について、以下で定める集合$\Stab(G, x)$を考える。
	このとき、以下で定める順序対$\qty(\Stab(G, x), \cdot)$は$G$の部分群である。
  \eq*{
  ...
  群準同型より、$\varphi(e, x) = \id_X(x) = x$、ゆえに部分モノイド。
...
\thm{類等式}{
  群$G$の有限集合$X$への群作用$\varphi$について、$n \in \N$と点列$\qty(x_m)_{m \in n} \in X^n$が存在して、以下の\num{2}つが成り立つ。
  \eqg*{
...
\thm{軌道・安定化部分群定理}{
  群$G$の$X$への群作用$\varphi$と元$x \in X$について、左剰余集合$G / \Stab(G, x)$を考える。
	このとき、全単射$f \colon G / \Stab(G, x) \to \varphi(G, x)$が存在する。
}{
  写像$\hat{f}_{x} \colon G \to X, \hat{f}_x(g) = \varphi(g, x)$を考える。
...
\lem{正規部分群の定める同値関係}{
  ...
  $\varphi$が定める同値関係$\sim_{\varphi(H)}$は演算と両立する同値関係である。
}{
...
\thm{群準同型定理}{
  群$G_1, G_2$と、群準同型$f \colon G_1 \to G_2$、$f$に付随する同値関係$\sim_{f}$について、群同型$\bar{f} \colon G_1 / \sim_{f} \to \Im(f)$が存在する。

  $G_1 / \sim_f = G_1 / \Ker(f)$であるため、$\bar{f} \colon G_1 / \Ker(f) \to \Im(f)$の形で書かれることが多い。
}{
  $\Im(f)$は$G_2$の部分群である。
...
\lem{剰余群の部分群は部分群の剰余群}{
  ...
  $\qty[\qty{e_Z}]^{-1} = N$であるため、$N \subseteq H$である。ゆえに$N$は$H$の正規部分群である。
...
\dfn{群の生成}{
  群$G$の部分集合$S$について、$S$が生成する部分群を以下で定めて、$\ev{S}$で表す。
  \eq*{
    \ev{S} \coloneqq \bigcap \qty{H \subseteq G \mid S \subseteq H \land \text{$H$は$G$の部分群}}
  }
}

\rem{生成する集合が単集合であるときの略記}{
  群$G$と元$g \in G$について、$\qty{g}$が生成する部分群$\ev{\qty{g}}$を、誤解がない範囲で単に$\ev{g}$で表す。
}

\dfn{群の生成系}{
  群$G$の部分集合$S$について、$G = \ev{S}$であるとき、$S$を$G$の生成系と呼ぶ。
}

\dfn{単生群}{
  群$G$について、以下を満たすとき、$G$を単生群と呼ぶ。
  \eq*{
    \exists g \in G \qty(G = \ev{g})
  }
}
...
\lem*{
	群$H, N$と、群準同型$\varphi \colon H \to \Aut(N)$について、$N \times H$上の以下で定義される演算$\cdot$を考える。
	\eq*{
...
\cor*{
	...
	\eq*{
		\forall \qty(n, h) \in N \rtimes H \qty(\qty(n, h) = \qty(n, e_H) \cdot \qty(e_N, h))
	}
}
...
\cor*{
	...
	\eq*{
		H' \coloneqq \qty{\qty(e_N, h) \mid h \in H}
	}
}
...
\lem*{
	...
	\eq*{
		N' \coloneqq \qty{\qty(n, e_H) \mid n \in N}
	}
}{
	明らかに部分群である。
