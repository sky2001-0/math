...
\cor*{
  ...
  \eq*{
    a \in I \rightarrow \ev{a} \subset I
  }
}
...
問題。
  ...
  $\P(\R)$上の演算$+, \times$を以下で定めると、$\P(\R)$は可換環をなす。
  \eqg*{
    A + B \coloneqq \qty(A \cup B) \setminus \qty(A \cap B) \\*
    A \times B \coloneqq A \cap B
  }

  このとき、多項式環の剰余環$\P(\R)[X] / \ev{\Q X + \Z}$は整域であるか？
...
\thm{PIDはNoether}{
  ...
  $a \in J$より、$\exists N \in \N \qty(a \in I_N)$より、$\ev{a} \subset I_N$

  $\forall N \in \N_{\geq N} \qty(\ev{a} \subset I_N \subset I_n \subset J = \ev{a})$
}
...
\thm{PIDはUFD}{
  ...
  \thmref{極大左イデアルの存在}、\thmref{極大イデアルは素イデアル}と$D$が単項イデアル整域であることより、素元$p_1$が存在して$a \in \ev{a} \subset \ev{p_1}$
  ...
  $a \in \ev{p(n)}$より、素イデアルの定義から$\exists k \in n' \qty(p'(k) \in \ev{p(n)})$である。ゆえに$\ev{p'(k)} \subset \ev{p(n)}$
