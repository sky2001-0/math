...
\dfnf{$\sigma$-加法族}{sigma-加法族}{
  空でない集合$X$について、以下を満たす集合系$\mathcal{S} \in \P(\P(X))$を$\sigma$-加法族と呼ぶ。
  \eqg*{
    X \in \mathcal{S} \\*
...
\dfn{Borel集合族}{
  ...
  \eq*{
    \mathcal{B}_X = \bigcap \qty{\mathcal{S} \in \P(\P(X)) \mid \text{$\mathcal{S}$は$\sigma$-加法族} \land \mathcal{O} \subset \mathcal{S}}
  }
}

\thm*{
  Borel集合族は$\sigma$-加法族である。すなわち、位相空間は可測空間である。
}{
  $T \coloneqq \qty{\mathcal{S} \in \P(\P(X)) \mid \text{$\mathcal{S}$は$\sigma$-加法族} \land \mathcal{O} \subset \mathcal{S}}$を考える。
...
\lem*{
  ...
  \thmref{原像の性質}より、$\forall A \in \P(\mathcal{T'}) \qty(\abs{A} \leq \aleph_0)$について、$f^{-1}(\bigcup A) = \bigcup \qty{f^{-1}(T') \mid T' \in A} \in \mathcal{S}$より、$\bigcup A \in \mathcal{T'}$である。
...
\dfn{拡大実数}{
  順序体$\R$について、以下を満たす２つの元$\infty, -\infty$を考える。
  \eqg*{
