...
\lem*{
  ...
  $0_\Z = \qty[\qty(0, 0)], 1_\Z = \qty[\qty(1, 0)]$であり、$\lnot \qty(\qty(0, 0) \sim_Z \qty(1, 0))$より明らか。
}
...
\thm{順序環としての整数}{
  ...
  $a = \qty[\qty(a_1, a_2)], b = \qty[\qty(b_1, b_2)], c = \qty[\qty(c_1, c_2)]$として、\lemref{自然数の順序の加法による保存}より、
  \eq*{
    a_1 + b_2 < a_2 + b_1 \rightarrow \qty(a_1 + c_1) + \qty(b_2 + c_2) < \qty(a_2 + c_2) + \qty(b_1 + c_1)
  }

  $0_\Z < a, 0_\Z < b$より、\dfnref{自然数の減法}より$a = \qty[\qty(a', 0)], b = \qty[\qty(b', 0)]$と置ける。
  \eq*{
