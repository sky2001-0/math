...
\thm{Lagrangeの定理}{
  ...
  写像$\varphi \colon G / H \times H \to G, \varphi(\qty[g], h) \coloneqq r(\qty[g]) h$を考える。

	\vskip\baselineskip

	$\varphi(\qty[g_1], h_1) = \varphi(\qty[g_2], h_2)$であるとき、$r(\qty[g_2]) = r(\qty[g_1]) h_1 h_2^{-1}$より、\lemref{群作用の軌道分解}から$\qty[g_1] = \qty[g_2]$であって、ただちに$h_1 = h_2$

	ゆえに$\varphi$は単射。

  \vskip\baselineskip

  $\forall g \in G$について、$r(\qty[g])^{-1} g \in H$であるので、$g = \varphi(\qty[g], r(\qty[g])^{-1} g)$と書ける。
	ゆえに$\varphi$は全射。
...
\thm{Lagrangeの定理の拡張}{
  ...
  \eq*{
	...
	\thmref{Lagrangeの定理}より、$\abs{G / K} \abs{K} = \abs{G} = \abs{G / H} \abs{H} = \abs{G / H} \abs{H / K} \abs{K}$である。

	$\abs{K} \neq 0$より、\lemref{自然数の乗法の簡約則}から成り立つ。
}

\lem{有限群の作用による軌道の大きさ}{
	有限群$G$と集合$X$について、$G$の$X$への作用$\varphi$を考える。

	このとき、$\forall x \in X$について軌道の要素数$\abs{\varphi(G, x)}$は$G$の約数である。
}{
  \thmref{Lagrangeの定理}、\thmref{軌道・安定化部分群定理}より以下が成り立つ。
  \eq*{
...
\lem{位数}{
	...
	\thmref{鳩の巣原理}より、写像$\tau \colon s(\abs{G}) \to G, \tau(n) = g^n$は単射でない。
...
\lem{巡回群の位数}{
	有限群$G$と元$g \in G$について、$G = \ev{g}$であるとき、以下が成り立つ。
	\eq*{
		\ord(g) = \abs{G}
	}
}{
	写像$\tau \colon \ord(g) \to G, \tau(n) = g^n$を考える。
...
\lem{素数群作用の不変部分}{
  ...
  $\abs{G}$が素数であるとき、$\abs{X} - \abs{\Fix(G, X)}$は$\abs{G}$の倍数である。
	...
	\lemref{有限群の作用による軌道の大きさ}と$\abs{G}$が素数であることより、$\abs{\varphi(G, x)} = 1 \lor \abs{\varphi(G, x)} = \abs{G}$である。
...
\lem{素数群は巡回群}{
	...
	\thmref{Lagrangeの定理}より、$\abs{H} = \abs{G}$である。
...
\thm{Sylowの第一定理}{
  ...
  $\abs{\Fix(G, G)}$が$p$の倍数でないとき、\thmref{類等式}より$\exists n \in \N \qty(\abs{G} = \abs{\Fix(G, G)} + \sum_{m \in n} \abs{\varphi(G, g_m)})$であるので、
	$\exists g \in G \st \abs{\varphi(G, g)} \neq 1 \land \abs{\varphi(G, g)}$は$p$の倍数でない、である。

	\thmref{軌道・安定化部分群定理}、\thmref{Lagrangeの定理}より、$\abs{G} = \abs{G / \Stab(G, g)} \abs{\Stab(G, g)} = \abs{\varphi(G, g)} \abs{\Stab(G, g)}$である。
  ...
  \lemref{Cauchyの定理}より、$\Fix(G, G)$は部分群$N$を持って$\abs{N} = p$であり、$\Fix(G, G)$が$G$の正規部分群であることから、$N$は$G$の正規部分群である。
	...
	剰余群$G / N$について、\thmref{Lagrangeの定理}より$\abs{G} = \abs{G / N} \abs{N}$であり、$\abs{G / N}$は$p^{n - 1}$の倍数であるので、仮定より部分群$K$が存在して、$\abs{K} = p^{n - 1}$である。

	\lemref{剰余群の部分群は部分群の剰余群}より、$G$の部分群$L$が存在して$L / N = K$であり、\thmref{Lagrangeの定理}より$\abs{L} = \abs{L / N} \abs{N} = \abs{K} \abs{N} = p^n$
}

\dfn{Sylow部分群}{
	素数$p$、自然数$n$、$p$の倍数ではない自然数$m$を考える。
	有限群$G$について、$\abs{G} = m p^n$であるとする。

	このとき、以下を満たす$G$の部分群$H$を、$G$のSylow $p$-部分群と呼ぶ。
	\eq*{
		\abs{H} = p^n
	}
}
...
\thm{Sylowの第二定理}{
	素数$p$、自然数$n$、有限群$G$、$G$のSylow $p$-部分群$H$を考える。

	$G$の部分群$K$について、$\abs{K} = p^n$であるとき、以下が成り立つ。
	\eq*{
		\exists g \in G \qty(K \subseteq \qty{g^{-1} h g \mid h \in H})
	}
}{
	$K$から$G$への左作用について、$\forall k \in K$についての写像$g \mapsto k g$は、$H$から$G$への右作用による軌道分解と両立する。

	\thmref{well-definedな写像}より、$K$から$G / H$への作用$\bar{\varphi}$が一意に存在する。

	\thmref{Lagrangeの定理}より、$\abs{G / H}$は$p$を約数に持たない。

	$G / H = \bigsqcup \qty{\bar{\varphi} \qty(K, \qty[g]) \mid \qty[g] \in G / H}$であるので、
	$\exists \qty[g] \in G / H \st \bar{\varphi} \qty(K, \qty[g])$は$p$を約数に持たない。

	\lemref{有限群の作用による軌道の大きさ}より、$\bar{\varphi} \qty(K, \qty[g])$は$\abs{K} = p^n$の約数である。
	よって、$\abs{\bar{\varphi} \qty(K, \qty[g])} = 1$である。

	したがって$\forall k \in K \qty(\qty[k g] = \qty[g])$である。

	すなわち$\forall k \in K$について$\exists h \in H$であり、$k g = g h$すなわち$k = g h g^{-1}$である。
}

\thm{Sylowの第三定理}{
	素数$p$、有限群$G$を考える。

	$G$のSylow $p$-部分群の全体$\mathcal{S}$について、以下が成り立つ。
	\eq*{
		\exists n \in \N \qty(\abs{\mathcal{S}} = n p + 1)
	}

	さらに、$G$の任意のSylow $p$-部分群$H$について、$H$の中心$\Fix(H, H)$を用いた以下が成り立つ。
	\eq*{
		\abs{G} = \abs{\mathcal{S}} \abs{\Fix(H, H)}
	}
}{
	Sylow $p$-部分群$H$を考える。
...
\dfn{置換}{
	$n \in \N_{\geq 1}$について、全単射$\sigma \colon n \to n$を置換と呼ぶ。
}
...
\dfn{置換の符号}{
	...
	このとき、以下の写像$\sign \colon \S_n \to \qty{-1, 1}$を定義する。
	\eq*{
		\sign(\sigma) =
		\begin{cases}
			1 & \qty(\text{偶数}) \\*
...
\cor*{
  ...
  \eq*{
    -1 \times -1 = 1 \times 1 = 1, \quad -1 \times 1 = 1 \times -1 = -1
  }
...
\dfn{交代群}{
	$n \in \N_{\geq 1}$について、以下で定める$\S_n$の部分群$A_n$を、交代群と呼ぶ。
	\eq*{
...
\lem{可解群の部分}{
	...
	埋め込み$f \colon Y_m \to Z_m$と、商写像$\qty[] \colon Z_m \to Z_m / Z_{s(m)}$から、準同型$\qty[] \circ f \colon Y_m \to Z_m / Z_{s(m)}$を得る。

	\thmref{群準同型定理}より、埋め込み$\bar{f} \colon Y_m / Y_{s(m)} \to Z_m / Z_{s(m)}$が存在する。
...
\lem{可解群の拡大}{
	...
	ゆえに自明な埋め込み$f \colon H_m / H_{s(m)} \to H_m / N$が存在する。
