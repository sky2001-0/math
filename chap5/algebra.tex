...
\dfn{二項演算}{
  集合$X$について、写像$\cdot \colon X^2 \to X$を、$X$上の二項演算、または$X$上の演算と呼ぶ。

  $\cdot(a, b)$を誤解のない範囲で$a \cdot b$とも表す。
}

\rem{二項演算と両立する同値関係}{
  集合$X$上の同値関係$\sim$について、誘導される$X^2$上の同値関係$\sim_2$を以下で定める。
  \eq*{
    \qty(x_1, x_2) \sim_2 \qty(y_1, y_2) \defiff x_1 \sim y_1 \land x_2 \sim y_2
  }

  $X$上の二項関係$\cdot$が同値関係$\sim$と両立するとは、$\cdot$が$\sim, \sim_2$と両立することである。
}

\dfn{マグマ}{
  集合$M$と$M$上の演算$\cdot$について、順序対$\qty(M, \cdot)$をマグマと呼ぶ。
  または単に$M$と書き、マグマを表すものとする。
}

\dfn{可換マグマ}{
  マグマ$\qty(M, +)$が以下を満たすとき、$M$を可換マグマと呼ぶ。
  可換であることを明示的に$+$で表す。
  \eq*{
...
\dfn{マグマ準同型}{
  マグマ$\qty(M_1, \cdot_{M_1}), \qty(M_2, \cdot_{M_2})$について以下を満たす写像$\varphi \colon M_1 \to M_2$が存在するとき、$\varphi$をマグマ準同型写像、または単にマグマ準同型と呼ぶ。
  \eq*{
...
\cor*{
  マグマ$M_1, M_2, M_3$と、マグマ準同型$f \colon M_1 \to M_2, g \colon M_2 \to M_3$について、合成写像$g \circ f$はマグマ準同型である。
}
...
\cor*{
  マグマ$M$について、恒等写像$\id_M$はマグマ同型である。
}
...
\dfn{部分マグマ}{
  ...
  \eq*{
    S \subseteq M \land \forall x, y \in S \qty(x \cdot y \in S)
  }
}

\cor*{
  マグマ$M_1, M_2$と、マグマ準同型$f \colon M_1 \to M_2$について、像$f(M_1)$は$M_2$の部分マグマである。
}

\dfn{商マグマ}{
  マグマ$\qty(M, \cdot)$を考える。

  演算$\cdot$と両立する同値関係$\sim$について、\thmref{well-definedな写像}より定める演算$\cdot'$が存在する。
...
\thm{マグマ準同型定理}{
  マグマ$\qty(M_1, \cdot_{M_1}), \qty(M_2, \cdot_{M_2})$とマグマ準同型$f \colon M_1 \to M_2$、および$f$に付随する同値関係$\sim_{f}$について、
  マグマ同型$\bar{f} \colon \qty(M_1 / \sim_{f}, \cdot) \to \qty(\Im(f), \cdot_{M_2})$が存在する。
}{
...
\thm{一般交換法則}{
  可換半群$\qty(S, +)$と点列$a \colon n \to S$を考える。
  任意の全単射$\sigma \colon n \to n$について、以下が成り立つ。
  \eq*{
...
\dfn{モノイド準同型}{
  モノイド$M_1, M_2$について、マグマ準同型$\varphi \colon M_1 \to M_2$が以下を満たすとき、$\varphi$をモノイド準同型写像、または単にモノイド準同型と呼ぶ。
  \eq*{
...
\cor{モノイド準同型の合成}{
  モノイド$M_1, M_2, M_3$と、モノイド準同型$f \colon M_1 \to M_2, g \colon M_2 \to M_3$について、合成写像$g \circ f$はモノイド準同型である。
}

\dfn{核}{
  モノイド準同型$\varphi \colon M_1 \to M_2$について、以下の原像を核と呼び、$\Ker(\varphi)$と表す。
  \eq*{
...
\thm{自然数は可換モノイド}{
  ...
  \lemref{自然数の加法の結合法則}、\lemref{自然数の加法の単位元}、\lemref{自然数の加法の交換法則}より、$\qty(\N, +)$は可換モノイドである。\vskip\baselineskip
...
\cor*{
  モノイド準同型$\varphi \colon M_1 \to M_2$について、$\Ker(\varphi)$は$M$の部分モノイドである。
}
...
\thm{モノイド準同型定理}{
  モノイド$M_1, M_2$と、モノイド準同型$f \colon M_1 \to M_2$、$f$に付随する同値関係$\sim_{f}$について、モノイド同型$\bar{f} \colon M_1 / \sim_{f} \to \Im(f)$が存在する。
}{
...
\dfn{総乗記号}{
  \mlemref{0}より定まる写像$\prod \colon M^\N \times \N \to M$を総乗記号と呼ぶ。
...
\dfn{総和記号}{
  可換モノイド$\qty(M, +)$と、集合$X$、写像$f \colon X \to M$について、以下を満たすとする。ただし、$0$は単位元とする。
  \eq*{
    \exists A \in \P(X) \qty(\abs{A} < \infty \land \forall x \in X \setminus A \qty(f(x) = 0))
  }

  このとき、全単射$\sigma \colon \abs{A} \to A$が存在する。集合についての総和記号を以下のように定義する。\thmref{一般交換法則}から一意に定まる。
  \eq*{
    \sum_{x \in X} f(x) \coloneqq \sum_{m \in \abs{A}} f(a_m)
  }
}

\dfn{指数}{
  モノイド$M$について、以下の写像$\text{\textasciicircum } \colon M \times \N \to M$を指数と呼ぶ。
  \eq*{
...
\lem{モノイド上の指数法則}{
  ...
  \begin{enumerate}
    \item $\forall x \in M$について、$\sigma \colon \qty(\N, +) \to M$であり、$\sigma(n) = x^n$
...
\lem{有限集合間の写像の全体}{
  空でない有限集合$X, Y$について、$Y^X$は有限であり、以下が成り立つ。
  \eq*{
    \abs{Y^X} = \abs{Y}^\abs{X}
  }
}{
  全単射$\sigma \colon Y^{\abs{X}} \to Y^X$が存在する。

  \lemref{自然数から集合への写像の全体}と\lemref{有限集合の直積}より、数学的帰納法を用いて$\abs{Y^{\abs{X}}} = \abs{Y}^{\abs{X}}$が成り立つ。
}
